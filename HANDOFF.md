...
- `bae7b16`에서 관심 정책·마감 일정에 정책명 검색과 ‘변경된 정책만 보기’를 추가했다. 일정의 접수 상태와 함께 필터하고, 검색 초기화·주소 복원·로그인 복귀·저장 해제 후 재조회에 연결했다.
- 웹 검사·검색/화면 주소/로그인 복귀 테스트·배포용 웹 빌드를 통과했다. 헤드리스 브라우저에서 검색 조합·빈 결과·초기화·뒤로/앞으로 가기·새로고침·새 탭·저장 해제 응답 유실·조회 실패·모의 로그인 복귀·키보드·모바일을 확인했다. 검색 중 추가 목록 조회가 없었고 검색 버튼의 공통 여백도 보완했다. [명령·범위·로그](docs/development/member-policy-flow.md#저장한-정책-검색)
...
- 검증 기준: 현재 웹 코드는 `bae7b16`에서 웹 검사·관련 테스트·헤드리스 검증·배포용 웹 빌드를 통과했다. 전체 서버·웹·생성 계약은 기준 `main`의 `24c924c`에서 원격 CI를 통과했다. 이후 서버·계약은 변경하지 않아 재사용했다. 조건 입력·질문·알림/이메일/계정 변경·관리자 처리는 해당 코드가 같아 기존 검증을 유지했다. 이후 문서만 변경했으며 미커밋 코드는 없다. 실제 bfcache 저장·복원 전체 과정·공급자 송수신·홈서버 이미지 실행·실제 검색 색인과 작업 브랜치의 원격 CI는 미검증이다.
...
내 정책의 화면 복원에 이어 저장 정책 검색·변경 필터를 완료했다.
