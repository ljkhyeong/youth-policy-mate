...
- 현재 `codex/policy-source-notices`의 `915b175`에서 OpenAI 호출을 기존 예산 예약·발송·복구 상태와 공고별 규칙 초안에 연결했다. `ai:policy-rules`에 실제 생성, 청구 확인, 무과금 해제를 추가했다. 입력 토큰 수·출력 상한·설정 단가로 최대 비용을 예약하고, 원 응답부터 보관해 초안 저장 실패 시 외부 호출 없이 다시 처리한다. 원문·요청 변경과 동일 요청의 중복 발송을 차단하며 자동 승인하지 않는다.
- `test:ai-rule-generation`과 전체 서버·DB·계약 검사를 통과했다. 가짜 HTTP 공급자와 실제 PostgreSQL을 사용했으며 명령·로그·미실행 항목은 [AI 규칙 추출](docs/development/ai-rule-drafts.md#검증)에 있다. 화면·HTTP 계약·생성 타입은 변경하지 않아 웹 검증은 `e05fa3d` 결과를 유지한다. 검증 후 변경은 문서뿐이다.
- 로컬 DB에 V27을 적용하고 정책 40건·적용 규칙 11건·AI 요청/호출 0건을 확인했다. Spring은 `915b175` 실행 파일·PID 3602·로그 `/tmp/youth-openai-rule-runtime.log`, Next는 PID 70721이다. AI 상태 조회 명령과 서버 상태 조회 200을 확인했다. 실제 AI 키·모델·요금·한도가 없어 OpenAI를 호출하지 않았다. 실행 시점에 프로세스를 재확인하며 이번 브랜치는 원격에 반영하지 않았다.
...
- AI: 공고 원문·OpenAI 호출·예산 예약·원 응답·규칙 초안을 연결했다. 응답 유실에는 예약을 유지하고 재호출하지 않는다. 실제 청구·무과금 확인 명령을 제공하며 자동 수집 연결·청구 자동 조회·실제 생성 품질 검증은 남아 있다. 모델·요금·월 한도는 설정값으로 받고 미설정 상태에는 호출하지 않는다. [규칙 추출과 설정](docs/development/ai-rule-drafts.md)·[요청 판단](docs/development/ai-request-admission.md)·[복구 실행](docs/development/ai-reservation-recovery-work-runs.md)
- 검증 기준: 전체 서버·DB·계약 검사는 `915b175`, 관리자 편집 화면·타입·모바일 검사는 `e05fa3d`다. 이번 작업은 화면과 API 계약을 유지해 웹 검사를 반복하지 않았다. 원격 CI·실제 관리자 로그인·실제 AI 품질/청구는 미검증이다.
...
1. OpenAI 모델·키·입출력 단가·요금 유효기간·월 AI 한도를 `.env`에 설정한 뒤 한 공고의 실제 생성 품질·청구를 확인한다. 수집 완료 후 신규/변경 공고의 자동 실행과 공급자 청구 조회는 남아 있다. 현재는 명시적 요청의 실제 호출·초안 저장·수동 정산까지 제공한다. 자동화에도 현재 요청 최신성·예산 경로를 재사용한다. 이전 공고의 연도만 바꾸거나 AI 결과를 바로 확정하지 않는다. 거주·취업·소득 재사용은 의미·기준일을 맞춘 뒤 확장한다.
