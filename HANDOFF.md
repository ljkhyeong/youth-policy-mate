...
- `eaef91c`에서 이메일 설정 조회·변경 오류 복구를 개선했다. 설정을 읽는 동안 변경을 막고, 응답 유실 시 이전 설정을 숨겨 재조회한다. 인증 코드 오류·요청 제한에는 입력을 유지하며 로그인 만료 시 개인 설정과 입력을 지운다.
- 웹 검사·회원 API 중계 테스트·운영 빌드를 통과했다. 헤드리스 브라우저에서 수정 전 오류를 재현하고 수신 동의·인증 메일·삭제 응답 유실, 재조회, 코드 오류·횟수 소진, 로그인 만료, 늦은 응답, 키보드 초점과 모바일을 확인했다. [명령·범위·로그](docs/development/email-settings-recovery.md#검증)
- 기존 3000 포트 개발 서버에서 확인했고 서버·DB·운영 환경은 변경하지 않았다. 최종 회귀 흐름의 변경 요청 12건은 모의 API에서만 처리했으며 검증 브라우저는 종료했다. 실제 회원 변경·외부 공급자 호출·이미지 빌드는 실행하지 않았다. 검증은 헤드리스로 수행했고 사용자 창·탭은 조작하지 않았다.
...
- 검증 기준: 웹의 현재 코드 기준은 `eaef91c`다. 전체 서버·웹·생성 계약은 기준 `main`의 `24c924c`에서 원격 CI를 통과했다. 이후 서버·계약은 변경하지 않아 재사용했고 이메일 설정 화면은 관련 검사와 브라우저로 확인했다. 회원 조건 화면의 `65ddd32` 검증도 코드가 같아 유지했다. 이후 문서만 변경했으며 미커밋 코드는 없다. 실제 공급자 송수신·홈서버 이미지 실행·실제 검색 색인과 작업 브랜치의 원격 CI는 미검증이다.
...
정책 저장·회원 조건·이메일 설정의 오류 복구 작업을 완료했다. 다음 코드 점검 대상은 `frontend/src/app/my/member-dashboard.tsx`의 저장 해제·로그아웃이다. 현재 변경 요청에 `AbortSignal`이 없으므로 요청 중 재조회·화면 이동과 늦은 응답 처리를 확인한다.
