...
- `d2b887f`에서 관리자 재처리·보정·규칙 등록/적용의 로그인 확인과 요청 취소를 공통 처리로 정리했다. 연속 제출과 화면 이동 후 늦은 변경 요청을 막는다. 결과 미확인 시 같은 요청번호·입력을 유지하는 기존 방식은 보존했다.
- 웹 검사·관리자 화면/규칙 편집/API 중계 테스트를 통과했다. 헤드리스 브라우저에서 다섯 관리자 작업의 중복·화면 이동·응답 유실·입력/권한/개정 오류·키보드·모바일을 확인했다. [명령·범위·로그](docs/development/admin-collection-exceptions.md#검증)
- 기존 3000 포트 개발 서버의 임시 검증 화면에서 실제 폼 컴포넌트를 확인했고, 검증 후 화면을 제거했다. 변경 요청 42건은 모의 API에서만 처리했으며 전용 브라우저는 종료했다. 서버·DB·운영 환경·사용자 창·탭은 변경하지 않았다. 실제 관리자 변경·외부 호출·이미지 빌드는 실행하지 않았다.
...
- 검증 기준: 웹의 현재 코드 기준은 `d2b887f`다. 전체 서버·웹·생성 계약은 기준 `main`의 `24c924c`에서 원격 CI를 통과했다. 이후 서버·계약은 변경하지 않아 재사용했다. 이번 관리자 변경은 클라이언트 요청 처리에 한정해 웹 검사·관련 테스트·브라우저로 확인했고 배포 빌드는 반복하지 않았다. 마지막 웹 빌드는 `a6586c5` 기준이며 조건·회원 화면의 기존 검증도 해당 코드가 같아 유지했다. 이후 문서만 변경했으며 미커밋 코드는 없다. 실제 공급자 송수신·홈서버 이미지 실행·실제 검색 색인과 작업 브랜치의 원격 CI는 미검증이다.
...
회원 화면의 오류 복구에 이어 관리자 재처리·보정·규칙 등록/적용에서 확인한 중복 요청과 화면 이탈 후 실행 문제를 처리했다.
