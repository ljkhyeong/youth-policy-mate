...
취업 조건의 입력 의미·자동 비교 전제·추가 확인 설계를 정리했다. 주된 상태 하나와 정책 코드 이름만으로 취업 요건을 확정하지 않는다. 기존 미확인 처리 원칙을 PRD 4~5절에 구체화했고, 이번에는 문서만 변경했다. 취업 비교기·추가 질문 화면·원천 코드 매핑은 아직 없으며 앱 테스트도 다시 실행하지 않았다.
...
- `npm run test:eligibility`: 직전 거주 비교 작업에서 거주 17건·연령 18건·집계 14건, 총 49건 및 서버 `assemble` 통과. API 인증키·DB·Docker가 필요하지 않다. 앞선 CI 구성 작업에서는 전체 서버 `build`의 당시 단위 14개·PostgreSQL 통합 2개가 모두 통과했다. 거주 비교 작업에서 웹·PostgreSQL 통합 검사는 다시 실행하지 않았다. 취업 조건 설계는 이 검증 범위에 포함하지 않는다.
...
- 취업 조건 설계에 따라, 정의·요구 방향·기준일이 확인된 단일 사실과 ‘해당·비해당·미확인’ 답변을 비교하는 순수 Java 모델부터 구현한다. 현재 화면의 주된 상태를 이 답변으로 자동 변환하지 않는다. 원천 코드 매핑·복합 조건·추가 질문 화면은 별도 작업이다.
