...
- `npm run test:preview-api`: 마감 API 4개·자격 API 3개·인공 답변 재판정 5개·공통 명세 1개, 총 13개. 이번 전체 서버 빌드에서 도메인 200개와 실제 DB·기본 차단 2개도 함께 실행해 총 215개가 통과했다.
...
- `npm run test:ingestion`: 수집 진행 12건과 AI 후보 12건, 총 24건. 부분 실패·명시적 종료·중단·재개·늦은 시도 결과·재전달·충돌·검토 필요와 AI 버전 관계를 확인한다. 인증키·DB·Docker 없이 실행한다. 실제 재시작·동시 작업자·중복 정책 반영 검증은 아니다.
...
- 다음 독립 작업 후보는 AI 요청 전 후보 재사용 여부와 비용 한도에 따른 호출 보류 기준이다. 공급자·요금·월 3만 원의 세부 배분은 미정이므로 실제 과금 차단이나 정해지지 않은 한도를 구현 완료로 표시하지 않는다.
...
- 현재 검증 범위는 개발 환경·DB 연결·접근 차단, 비회원 입력·인공 예시 표시, 서버의 조건 비교·모집·후보 계산, 개발용 자격·마감 API와 생성 계약·화면 연결·실패 복구, 내부 개정 적용 판단·수집 진행·AI 후보 버전 검사다. 실제 수집·원본 저장·DB 중복 방지·정책 정확도·외부 연동·소셜 인증·예약·발송 검증으로 확대해서 보고하지 않는다.
