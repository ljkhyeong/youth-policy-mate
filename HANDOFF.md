...
소득의 개인·가구 범위, 금액 정의·단위·대상 기간·기준표와 구간 경계 설계를 정리했다. 기존 미확인 원칙을 PRD 4~5절에 구체화했으며 소득 구간을 대표 금액으로 바꾸지 않는다. 서울시 상반기 이사비 공고의 보험료 기준을 보류 사례로 확인했지만 원천 수집원이나 금액표를 추가하지 않았다. 이번에는 문서만 바꿨고 소득 비교기·입력 화면은 아직 없으며 앱 테스트·빌드는 다시 실행하지 않았다.
...
- 소득 조건 설계에 따라 같은 정의·대상·기간의 금액 구간과 명시적인 정책 범위를 비교하는 순수 Java 모델을 구현한다. 포함 경계, 구간 일부 겹침, 제한 없음·미해석·다른 기준의 입력을 구분한다. 기준표 계산·월연 환산·원천 코드 매핑·소득 UI는 별도 작업이다. 기존 취업 질문 미리보기와 실제 정책 연결도 구분한다.
