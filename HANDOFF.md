...
- `31b0366`에서 내 정책의 목록 조회·저장 해제·로그아웃·탈퇴 요청을 함께 관리하도록 정리했다. 중복 요청을 막고, 변경 결과가 불명확하면 개인 상태를 숨겨 재조회한다. 화면 이동 후 늦은 응답으로 현재 화면이나 위치를 바꾸지 않는다.
- 웹 검사·저장 정책 표시/로그인 복귀 테스트·운영 빌드를 통과했다. 헤드리스 브라우저에서 늦은 로그아웃 응답 문제를 재현하고 요청 중복·조회 복구·로그인 만료·탈퇴 확인/실패/성공·화면 이동·다른 탭 알림·키보드 초점·모바일을 확인했다. [명령·범위·로그](docs/development/member-dashboard-recovery.md#검증)
- 기존 3000 포트 개발 서버에서 확인했고 서버·DB·운영 환경은 변경하지 않았다. 최종 회귀 흐름의 변경 요청 11건은 모의 API에서만 처리했으며 검증 브라우저는 종료했다. 실제 회원 변경·외부 공급자 호출·이미지 빌드는 실행하지 않았다. 검증은 헤드리스로 수행했고 사용자 창·탭은 조작하지 않았다.
...
- 검증 기준: 웹의 현재 코드 기준은 `31b0366`다. 전체 서버·웹·생성 계약은 기준 `main`의 `24c924c`에서 원격 CI를 통과했다. 이후 서버·계약은 변경하지 않아 재사용했고 대시보드·탈퇴 화면은 관련 검사와 브라우저로 확인했다. 회원 조건·이메일 설정의 기존 검증도 해당 코드가 같아 유지했다. 이후 문서만 변경했으며 미커밋 코드는 없다. 실제 공급자 송수신·홈서버 이미지 실행·실제 검색 색인과 작업 브랜치의 원격 CI는 미검증이다.
...
정책 저장·회원 조건·이메일 설정·대시보드의 오류 복구 작업을 완료했다. 다음 코드 점검 대상은 `frontend/src/app/login/login-panel.tsx`의 로그인 방법 조회 실패와 복귀 경로다. 현재 조회 실패 안내에는 재시도 버튼이 없다.
