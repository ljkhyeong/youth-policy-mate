...
- `npm run test:preview-api`: 마감 API 4개·자격 API 3개·인공 답변 재판정 5개·공통 명세 1개, 총 13개. 이번 전체 서버 빌드에서 도메인 227개와 PostgreSQL 예약·실행·연결/기본 차단 21개도 함께 실행해 총 261개가 통과했다.
...
- 빌드와 PostgreSQL 예약·실행·연결/기본 차단 테스트 21개가 통과했다. 로컬 서버의 상태 응답 200과 차단 경로 403, 시작 화면은 앞선 작업에서 확인했다.
...
- 다음 독립 작업 후보는 미완료 예약의 작업자 소유권·호출 직전 중단·재확인 이력을 설계하고, 인공 실행 결과를 현재 정책 개정과 비교해 후보 상태에 반영하는 것이다. 실제 후보 DB 저장은 정책·원본 저장 구조가 생긴 뒤 연결한다.
...
- 현재 검증 범위는 개발 환경·DB 연결·접근 차단, 비회원 입력·인공 예시 표시, 서버의 조건 비교·모집·후보 계산, 개발용 자격·마감 API와 생성 계약·화면 연결·실패 복구, 내부 개정 적용 판단·수집 진행·AI 후보 버전·사전 비용·메모리 예약 상태, PostgreSQL 예약·내부 상태 전이와 인공 실행 오케스트레이션이다. 실제 수집·정책 원본 저장·AI 외부 연동·공급자 청구 확인·소셜 인증·알림 예약·발송·과금 차단 검증으로 확대해서 보고하지 않는다.
