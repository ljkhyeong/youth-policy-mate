...
- `cc71286`에서 상반기 이사비 지원의 소득 확인 질문을 추가했다. 공고 보험료 기준 이하·초과와 부양자·가구 기준·대체 증빙 미확인을 구분한다. 규칙은 `moving-fee-2026-h1-v2`이며 구버전 답변은 차단한다. [검증 명령·로그·화면 확인](docs/development/moving-fee-questions.md#검증)을 통과했으며 이후 변경은 문서뿐이다.
...
- 조건 질문: 국가근로장학금·응시료 지원·K-패스·청년주택드림청약통장·서울청년정책네트워크·상반기 이사비 지원 6개 정책을 제공한다. 이사비는 2026년 3월 보험료의 공고 기준 비교 답변을 받으며 증빙 심사는 별도다. 일부 요건만 비교하므로 전체 자격은 추가 확인 필요다. [질문 탐색](docs/development/policy-question-discovery.md)·[이사비 질문](docs/development/moving-fee-questions.md)
...
1. 다른 정책의 원문·예외를 검토해 질문과 기본 조건 비교 범위를 늘린다. 청년내일저축계좌(`20260430005400113009`)는 저장 원문의 가구소득 100%·50% 안내가 충돌한다. [공식 2026년 공고](https://www.bokjiro.go.kr/ssis-tbu/cms/pc/customer/notice/1309680_1141.html)는 중위소득 50% 이하로 안내하므로 질문 추가를 보류했다. 원문은 수정하지 않았으며 이 충돌을 먼저 재검토한다.
