...
- 현재 `codex/policy-source-notices`의 `5ab882d`에서 청년내일저축계좌 연령을 내 조건 검색·정렬에 연결했다. 상세 질문과 같은 2026년 5월 모집의 출생일 기준을 사용하며 마감·수집 안내와의 기준 차이를 표시한다. 다른 조건과 전체 자격은 추가 확인으로 유지한다.
- 이번 검증의 명령·로그·실제 화면·제약은 [개인 조건 탐색](docs/development/condition-policy-discovery.md#검증)에 있다. 로컬 변경은 커밋했으며 이번 브랜치는 원격에 반영하지 않았다.
...
- 검증 기준: `1b7fbdd`의 전체 CI 이후 상세 충돌 안내와 저축계좌 질문·기본 연령 비교를 추가했다. `5ab882d`의 저축계좌 규칙·PostgreSQL 정책 API·서버 패키징과 실제 화면을 확인했다. API 구조·생성 타입·웹 코드는 `676fe5d` 검증 이후 같아 해당 웹 검사를 재사용한다. 이후 변경은 문서뿐이며 DB V20·실제 정책 데이터는 유지했다.
