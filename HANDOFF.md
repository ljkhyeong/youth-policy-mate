...
- `codex/condition-policy-discovery`의 `5286fad`에서 관리자 수집 예외 화면과 로그인 복귀를 연결했다. 서버 API는 `df50b0f`와 같다. 개인 조건 검색·정렬, 이사비 질문, 접수 상태 표시·필터, 청약통장 기본 연령 비교가 포함된 브랜치이며 아직 `main`에 병합하지 않았다.
...
- 관리자 수집 예외: `/admin/collection-exceptions`에서 실패 목록·원본·현재 공개 내용을 조회하고 로그인 후 목록으로 복귀한다. 명시한 회원 UUID의 소셜 세션만 허용하고 기본 차단한다. PC·모바일 표시·페이지 복귀·조회 오류·권한 변경 후 뒤로 가기를 검증용 응답으로 확인했다. 실제 소셜 계정 연결·보정·재처리 API는 아직 없다. [설정과 계약](docs/development/admin-collection-exceptions.md)
...
- 최근 검증: `5286fad`의 관리자 화면·로그인 복귀 관련 웹 테스트, 최종 린트·타입 검사, 배포 빌드와 브라우저 흐름을 통과했다. 빌드 이후 린트 예외 주석만 제거했다. 서버·계약은 바뀌지 않아 `df50b0f`의 전체 서버 검사·생성 타입 검사를 재사용했다. 꺼진 DB·개발 서버를 다시 실행해 정책 40건과 실제 로그인 안내를 확인했다. 실제 관리자 소셜 로그인은 미검증이다. [명령·범위·결과](docs/development/admin-collection-exceptions.md#검증)를 참고한다.
...
3. 관리자 페이지 요청 실패 조회·이전 개정 비교·근거를 남기는 보정과 재처리를 구현한다.
