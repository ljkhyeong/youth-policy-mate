...
- `65ddd32`에서 회원 조건 화면의 조회 오류와 요청 취소 처리를 개선했다. 회원 조회 중·조회 실패·비회원을 구분하고 재시도를 제공한다. 조건 조회 실패·빈 결과에도 입력을 유지하며, 중복 요청과 이전 화면의 늦은 응답을 막고 로그인 만료 시 재로그인을 안내한다.
- 웹 검사·조건 입력/생년월일 재사용/로그인 복귀 테스트·운영 빌드를 통과했다. 헤드리스 브라우저에서 조회·변경 오류, 재시도, 입력 보존, 로그인 만료, 중복 요청, 화면 전환, 키보드 초점과 모바일을 확인했다. [명령·범위·로그](docs/development/condition-member-recovery.md#검증)
- 기존 3000 포트 개발 서버에서 확인했고 서버·DB·운영 환경은 변경하지 않았다. 조건 요청 10건은 모의 API에서만 처리했으며 검증 브라우저는 종료했다. 실제 회원 변경·외부 공급자 호출·이미지 빌드는 실행하지 않았다. 브라우저 검증은 헤드리스로 수행하고 사용자 창·탭을 조작하지 않도록 `AGENTS.md`에 기록했다.
...
- 검증 기준: 웹의 현재 코드 기준은 `65ddd32`다. 전체 서버·웹·생성 계약은 기준 `main`의 `24c924c`에서 원격 CI를 통과했다. 이후 서버·계약은 변경하지 않아 재사용했고 조건 화면 변경은 관련 검사와 브라우저로 확인했다. 이후 문서만 변경했으며 미커밋 코드는 없다. 실제 공급자 송수신·홈서버 이미지 실행·실제 검색 색인과 작업 브랜치의 원격 CI는 미검증이다.
...
정책 저장과 회원 조건 화면의 오류 복구 작업을 완료했다. 다음 항목은 실제 연동·운영 설정이 필요한 작업이며, 관련 설정은 사용자가 진행한다.
