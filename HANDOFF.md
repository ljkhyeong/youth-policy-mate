...
이번에는 읽기 전용 마감·알림 후보 컴포넌트와 개발 전용 `/dev/reminders`를 추가했다. 일곱 가지 고정 인공 자료로 날짜형·시각형 기간, 원문·서울 시간대, 오늘 후보와 후보 없음 사유를 구분한다. 화면에서 날짜를 다시 계산하거나 실제 서버·저장·예약·발송과 연결하지 않는다. 웹 32건·린트·타입 검사·빌드, 개발 200·운영 404, 예시 전환·근거·새로고침 초기화와 모바일·데스크톱 표시를 확인했다. 키보드 전용 전체 흐름은 도구 입력 한계로 미확인이다. 서버·PostgreSQL 통합 검사는 다시 실행하지 않았다.
...
- `npm run test:web`: Vitest 테스트 32개. 기본 입력 5개, 공통 상태 3개, 취업 질문 4개, 소득 질문 5개, 자격 결과·근거 7개, 마감·알림 후보 표시 8개를 확인한다. 이번 후보 화면 작업에서 테스트·린트·타입 검사·프로덕션 빌드를 통과했다.
...
- `frontend/src/features/reminders/`·`frontend/src/app/dev/reminders/`: 마감과 알림 후보의 읽기 전용 컴포넌트·표시 모델·인공 예시. 날짜형 비변환, 시각형 시간대, 오늘 후보·빈 후보 사유와 계산 근거를 표시한다. 운영 404와 모바일·데스크톱을 확인했으며 서버 API·실제 예약 연결은 없다. [미리보기 검증 기록](docs/development/deadline-reminder-preview.md)에 키보드·요소 조회 도구의 한계를 구분했다.
...
- 다음은 개발 전용 인공 자료 조회 API로 모집 상태·알림 후보의 서버 계산과 화면 연결을 준비한다. 먼저 API 계약·생성 타입 범위와 운영 모드 차단을 정한다. 원래 기간·시간대·개정·근거·계산 기준을 보존하고 화면 계산은 추가하지 않는다. 실제 정책·회원 저장·예약·발송은 별도이며 발송 시각·재시도 만료도 아직 미정이다. 원천 기간 파서는 성공 응답 확보 후 작성한다.
...
- 현재 검증 범위는 개발 환경·DB 연결·접근 차단, 비회원 조건 입력·인공 예시의 질문·자격 결과·마감 후보 표시, 서버의 자격 조건 비교·집계, 확인된 모집 기간과 마감 알림 후보 날짜 계산이다. 실제 정책 판정·원문 날짜 해석의 정확도, 외부 연동, 소셜 인증이나 알림 예약·발송 검증으로 확대해서 보고하지 않는다.
