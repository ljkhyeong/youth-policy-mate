...
- `f6b63f2`에서 저장 조건의 늦은 응답이 직접 수정·전체 삭제한 입력을 덮어쓰는 문제를 수정했다. 소셜 생년월일도 직접 수정·삭제한 뒤에는 적용하지 않는다. 입력 중 서버 재조회는 추가하지 않았으며 명시적으로 다시 불러오기는 허용한다.
- 웹 검사·조건 입력/생년월일 메모리 테스트를 통과했다. 헤드리스 브라우저에서 수정 전 문제를 재현하고 입력 보존·초점·정상 불러오기와 기존 저장·삭제·오류·로그인 만료·화면 이동·모바일을 확인했다. [명령·범위·로그](docs/development/condition-member-recovery.md#검증)
- 기존 3000 포트 개발 서버의 `/conditions`에서 회원 API를 모의 처리했고 실제 회원 변경·외부 호출은 없었다. 임시 화면을 추가하지 않았으며 전용 브라우저를 종료했다. 서버·DB·운영 환경·사용자 창·탭은 변경하지 않았다. 이미지 빌드는 실행하지 않았다.
...
- 검증 기준: 현재 웹 코드는 `f6b63f2`에서 웹 검사·관련 테스트·헤드리스 검증을 통과했다. 마지막 배포 빌드는 `8d650c9`이며 이번 클라이언트 이벤트 수정에서는 반복하지 않았다. 전체 서버·웹·생성 계약은 기준 `main`의 `24c924c`에서 원격 CI를 통과했다. 이후 서버·계약은 변경하지 않아 재사용했다. 질문·다른 회원 화면·관리자 요청 처리는 코드가 같아 기존 검증을 유지했다. 이후 문서만 변경했으며 미커밋 코드는 없다. 실제 공급자 송수신·홈서버 이미지 실행·실제 검색 색인과 작업 브랜치의 원격 CI는 미검증이다.
...
조건 질문의 요청 충돌에 이어 기본 조건 화면에서 확인한 직접 입력·전체 삭제의 덮어쓰기를 처리했다. 조건 입력 문서도 현재 연령 비교·회원 저장 기능에 맞췄다.
