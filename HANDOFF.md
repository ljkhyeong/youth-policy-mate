...
서울 대상 MVP의 제품 범위와 기술 스택을 합의하고 기준 문서·전용 스킬, Next.js·Spring Boot·로컬 PostgreSQL 기본 구성을 작성했다. 개발 준비 화면과 서버 상태 확인을 제공한다. 정책 API, 업무 DB 스키마, 수집·판정·인증·저장·알림 기능은 아직 구현하지 않았다.
...
- `frontend/`: Next.js App Router·React·TypeScript·Tailwind. 시작 화면은 준비 안내이며 정책 API를 호출하지 않는다.
...
- 현재 검증 범위는 개발 환경과 DB 연결·접근 차단이다. 정책 판정 정확도, 외부 연동, 소셜 인증이나 알림 발송 검증으로 확대해서 보고하지 않는다.
