...
- `npm run test:preview-api`: 마감 API 4개·자격 API 3개·인공 답변 재판정 5개·공통 명세 1개, 총 13개. 이번 전체 서버 빌드에서 DB 없는 도메인 246개와 PostgreSQL 예약·실행·복구·연결·운영 조회·수동 검토 재개·작업 배정·제한 목록·작업 실행 기록/운영 중단/실행-시도 연결/기본 차단 75개도 함께 실행해 총 334개가 통과했다.
...
- 같은 패키지의 `AiReservationRecoveryLeaseRenewalStore`: 활성 시도의 현재 만료를 같은 작업자만 연장하고 갱신 ID·시도 순번·이전/새 만료 시각을 별도 감사 기록에 저장한다. 자동 heartbeat와 고정 갱신 주기는 없다.
...
- `npm run test:ai-recovery-execution`: PostgreSQL과 인공 복구 포트 15건. 기존 다음 예약 획득과 배정된 시도 실행, 트랜잭션 밖 확인, 응답과 청구 대기·정산·무과금·취소, 완료·교체·만료·갱신 임대와 변경·종료 예약의 결과 차단을 확인한다. Docker가 필요하다.
...
- 빌드와 PostgreSQL 예약·실행·복구·임대 갱신·연결·운영 조회·수동 검토 재개·작업 배정·제한 목록·작업 실행 기록/운영 중단/실행-시도 연결/기본 차단 테스트 81개가 통과했다. 전체 서버 340건이며 로컬 서버의 상태 응답 200과 차단 경로 403, 시작 화면은 앞선 작업에서 확인했다.
...
- 활성 임대 갱신의 소유자·시도 순번·시각 펜싱과 감사 기록을 구현했다. 다음 독립 작업 후보는 복구 작업자가 외부 확인 중 자동 heartbeat를 호출하고 갱신 실패 시 결과 적용을 중단하는 조정 경계를 정하는 것이다. 실제 공급자 조회·운영 일정·고정 재확인 값은 공급자 선정 전까지 넣지 않는다.
...
- 현재 검증 범위는 개발 환경·DB 연결·접근 차단, 비회원 입력·인공 예시 표시, 서버의 조건 비교·모집·후보 계산, 개발용 자격·마감 API와 생성 계약·화면 연결·실패 복구, 내부 개정 적용 판단·수집 진행·AI 후보 버전·사전 비용·메모리 예약 상태, PostgreSQL 예약·내부 상태 전이·인공 실행·복구 소유권·펜싱·활성 임대 갱신 감사·읽기 전용 운영 조회·수동 검토 재개 감사·작업 배정·배정 실행·제한 목록 실행·작업 실행 기록·오래된 실행 조회와 운영 중단·실행과 복구 시도의 직접 연결, 실행·복구 응답의 현재 후보 재검사다. 실제 수집·정책 원본 저장·AI 외부 연동·자동 heartbeat·후보 DB 저장·공급자 청구 확인·관리자 인증·소셜 인증·알림 예약·발송·과금 차단 검증으로 확대해서 보고하지 않는다.
