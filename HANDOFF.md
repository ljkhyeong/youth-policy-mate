...
- 현재 `codex/policy-source-notices`의 `f189078`에서 회원 알림 조회를 개선했다. 최근 100건 제한을 없애고 페이지 조회·안 읽은 알림 수와 필터·한국 시간 표시를 제공한다. 알림 조회와 오류를 관심 정책·마감 일정에서 분리했고 이메일 설정은 펼침 메뉴로 정리했다.
- 회원 PostgreSQL 통합·관련 웹·계약·최종 린트·타입·배포 빌드를 통과했다. 브라우저에서 페이지 이동·읽음 처리·오류 복구·늦은 응답 취소·390px/1280px 표시·키보드를 테스트 응답으로 확인했다. 명령·로그·검증 이후 파일 차이는 [회원 알림 조회](docs/development/member-notifications.md#검증)에 있다. 마지막 코드 변경까지 검증했으며 이후 변경은 문서뿐이다.
- 로컬 Spring은 `f189078` 실행 파일·PID 67958·로그 `/tmp/youth-member-notifications-runtime.log`, Next는 PID 10257·로그 `/tmp/youth-admin-ai-web.log`다. 상태 조회 200·알림 API 비회원 401·내 정책 화면 200을 확인했다. DB 마이그레이션은 V28을 유지했고 실제 회원 데이터를 변경하지 않았다. AI 키·모델·요금·한도는 미설정이다. 자동 추출·정기 수집·이메일·알림은 비활성화했고 실제 AI 호출은 실행하지 않았다. 실행 시점에 프로세스를 재확인하며 이번 브랜치는 원격에 반영하지 않았다.
...
- 회원 기능: OAuth 로그인 코드, 관심 정책 저장·해제, 일정·[서비스 내 알림 페이지와 미읽음 필터](docs/development/member-notifications.md)를 연결했다. [마감 일정](docs/development/member-calendar.md)에서 접수 상태를 구분하고 가까운 마감순 정렬·상태 필터·새로고침을 제공한다. 테스트 제공자와 실제 HTTP·DB로 코드 교환부터 관리자 접근·세션 종료까지 확인했다. 로컬 카카오·네이버 ID/Secret과 관리자 ID가 없어 실제 제공자 로그인은 미검증이다. [회원 기능](docs/development/member-policy-flow.md)
...
- 검증 기준: 회원·일정·알림 통합과 관련 웹·계약은 `f189078`, 관리자 API·웹은 `280a12b`, 변경하지 않은 자동 추출·생성·비용 처리 등 전체 서버 기준은 `ee34aca`다. 실제 제공자 로그인·AI 품질/청구·원격 CI는 미검증이다.
