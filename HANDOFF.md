...
- `72c8013`에서 탭 간 통신 미지원 시 보관 화면 복원 처리도 빠지는 문제를 수정했다. 복원 이벤트는 항상 등록하고, 계정 변경 알림과 같은 함수로 생년월일 메모리를 지운 뒤 화면을 다시 조회한다.
- 웹 검사·생년월일 메모리/로그인 복귀 테스트를 통과했다. 헤드리스 브라우저에서 통신 미지원 상태의 복원 이벤트를 재현하고 탭 간 로그인·로그아웃, 이전 조건 응답 무시, 중복 새로고침 방지를 확인했다. 실제 bfcache 저장·복원 과정 전체는 미검증이다. [명령·범위·로그](docs/development/member-policy-flow.md#계정-전환과-보관-화면-복원-검증)
- 기존 3000 포트 개발 서버의 `/conditions`·`/my`·로그인 완료 화면에서 회원 API를 모의 처리했고 실제 회원 변경·외부 호출은 없었다. 임시 화면을 추가하지 않았으며 검증 탭과 전용 브라우저를 종료했다. 서버·DB·운영 환경·사용자 창·탭은 변경하지 않았다. 이미지 빌드는 실행하지 않았다.
...
- 검증 기준: 현재 웹 코드는 `72c8013`에서 웹 검사·관련 테스트·헤드리스 검증을 통과했다. 마지막 배포 빌드는 `8d650c9`이며 이번 클라이언트 이벤트 수정에서는 반복하지 않았다. 전체 서버·웹·생성 계약은 기준 `main`의 `24c924c`에서 원격 CI를 통과했다. 이후 서버·계약은 변경하지 않아 재사용했다. 조건 입력·질문·다른 회원 화면·관리자 요청 처리는 해당 코드가 같아 기존 검증을 유지했다. 이후 문서만 변경했으며 미커밋 코드는 없다. 실제 공급자 송수신·홈서버 이미지 실행·실제 검색 색인과 작업 브랜치의 원격 CI는 미검증이다.
...
기본 조건의 입력 덮어쓰기에 이어 계정 전환과 보관 화면 복원에서 확인한 초기화 누락을 처리했다.
