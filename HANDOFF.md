...
- `npm run test:eligibility`: 소득 47건·취업 21건·거주 17건·연령 18건·집계 14건, 총 117건. 이번 모집 기간 작업에서 23건의 새 테스트와 함께 총 140건 및 서버 `assemble`을 실행해 모두 통과했다. API 인증키·DB·Docker가 필요하지 않다. 앞선 CI 구성 작업에서 통과한 PostgreSQL 통합 2건은 이번에 다시 실행하지 않았다.
- `npm run test:recruitment`: 모집 기간 23건. 서울 자정·시각 경계, 같은 날 모집, 미확인 이유와 근거·개정 보존, 시계 시간대 독립성과 한 번 읽기, 잘못된 범위 거절을 확인한다. 실행·설계 범위는 [모집 기간 구현](docs/development/recruitment-period.md)을 따른다.
...
- 다음은 확인된 신청 마감에서 D-7·D-3·D-1 후보 날짜를 계산하는 작은 일정 모델을 준비한다. 서울 날짜·원본의 날짜/시각 구분을 보존하고 상시·소진 시 종료·미확인에는 후보를 만들지 않는다. 발송 시각·재시도 만료는 아직 미정이므로 실제 예약·발송·저장 기능으로 확대하지 않는다. 모집 기간의 원천 파서는 성공 응답 확보 후 작성하며 결과 화면의 모집 안내는 여전히 고정 예시다.
...
- 현재 검증 범위는 개발 환경·DB 연결·접근 차단, 비회원 조건 입력·인공 예시의 추가 질문과 결과 표시, 서버의 판정 결과 집계와 명시적 연령·거주·단일 취업·소득 구간 비교, 확인된 모집 기간 계산이다. 실제 정책 판정·원문 날짜 해석의 정확도, 외부 연동, 소셜 인증이나 알림 발송 검증으로 확대해서 보고하지 않는다.
