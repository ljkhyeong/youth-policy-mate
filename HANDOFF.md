...
- 현재 `codex/policy-source-notices`의 `df2efb1`에서 관리자 규칙 파일 등록·검토 후 적용을 연결했다. `/admin/collection-exceptions/rules`의 상세에서 파일·등록/적용 사유·작업자를 확인한다. 원문·기간·개정·현재 버전 검사와 요청 ID 재시도를 적용하고 작업 이력 실패 시 함께 롤백한다. 조건 파일의 편집은 외부 편집기를 사용한다. 기존 11개 정책의 실제 질문·판정 데이터는 변경하지 않았다.
- 전체 서버·DB·OpenAPI 계약, 관련 관리자 웹 테스트·타입 검사·배포 빌드를 통과했다. 별도 테스트 API와 브라우저로 등록·적용 응답 유실 후 재확인·작업 중복 없음·파일 일치·409 차단·키보드·모바일을 확인했다. 명령·로그·미실행 범위는 [관리자 조건 검토·적용](docs/development/policy-rule-review.md#검증)에 있다. 전체 검사 후 코드 변경은 없으며 문서 커밋으로 같은 검사를 반복하지 않는다.
- 로컬 DB는 V25·정책 40건·적용 규칙 11건이다. Spring은 PID 78755, 로그 `/tmp/youth-rule-management-runtime.log`, Next는 PID 70721이다. V25 적용·정상 기동과 규칙 파일 API의 비회원 401을 확인했다. 실행 시점에 재확인한다. 이번 브랜치는 원격에 반영하지 않았다.
...
- 검증 기준: 최신 전체 서버·DB·계약·웹 검증은 `df2efb1` 구현 기준이다. 관리자 규칙 등록·적용·파일 API와 생성 타입을 추가했고 공개 정책 API는 유지했다. 원격 CI·실제 관리자 로그인은 이번 작업에서 실행하지 않았다.
