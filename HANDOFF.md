...
- 개인 조건 탐색: 햇살론유스를 포함한 7개 정책의 연령을 비교하고 전체 정책에서 검색·정렬한다. 정책별 기준일·출생일 범위·병역 예외를 구분하며 햇살론유스는 오늘 보증신청을 가정한다. 거주·취업·소득은 추가 확인으로 남긴다. 검색·페이지 이동·오류 재시도·모바일 화면을 로컬 실제 정책으로 확인했다.
...
1. 다른 정책의 원문·예외를 검토해 질문과 기본 조건 비교 범위를 늘린다. 현재 질문은 일부 조건만 비교하며 보증·대출 심사, 보증료 지급액·예산, 저축계좌 선정·가입 후 유지 등은 남아 있다. 원문이 바뀌면 질문과 충돌 안내를 재검토한다.
