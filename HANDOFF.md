...
이번에는 개발 전용 `/dev/income`에 소득 대상·기간·단위, 구간·모름 선택과 확인·수정·삭제를 추가했다. 기간 변경 시 이전 답변을 지우고, 구간 추가 확인은 현재 답변의 판정이 아닌 별도 인공 상황으로 보여준다. 실제 소득 입력·서버 전송·저장·판정 연결은 없다. 웹 17건·린트·타입 검사·빌드, 개발 200·운영 404와 모바일·데스크톱 흐름을 확인했다. 서버 검사는 다시 실행하지 않았다.
...
- `npm run test:web`: Vitest 테스트 17개. 기본 입력 검사 5개, 상태 안내·오류 원문 비노출·404 복귀 경로 3개, 취업 질문·답변 표시 4개, 소득 질문·답변 표시 5개를 확인한다. 소득 질문 화면 작업에서 테스트·린트·타입 검사·프로덕션 빌드를 통과했다.
...
- `npm run test:eligibility`: 소득 47건·취업 21건·거주 17건·연령 18건·집계 14건, 총 117건. 소득 비교기 작업에서 같은 테스트 범위와 서버 `assemble`을 한 Gradle 명령으로 실행해 모두 통과했다. API 인증키·DB·Docker가 필요하지 않다. 앞선 CI 구성 작업에서는 전체 서버 `build`의 당시 단위 14개·PostgreSQL 통합 2개가 모두 통과했다. 이번 소득 질문 화면 작업에서 서버·PostgreSQL 통합 검사는 다시 실행하지 않았다.
...
- 다음은 항목별 자격 결과와 근거를 표시하는 화면 컴포넌트를 준비한다. 인공 자료로 충족·불충족·사용자 정보 부족·정책 미해석의 표시를 점검하되 실제 정책 추천으로 노출하지 않는다. 화면이 자격 상태를 재계산하지 않게 하고 모집 상태와 자격 안내도 구분한다. 실제 정책·서버 결과 연결과 확정 API 계약은 별도다.
