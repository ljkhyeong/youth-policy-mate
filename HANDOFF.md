...
이번에는 읽기 전용 자격 결과·근거 컴포넌트와 개발 전용 `/dev/eligibility`를 추가했다. 네 가지 고정 인공 결과로 전체 상태·모집 상태·검토 미완료·항목별 충족/불충족/미확인·근거·기준 정보를 표시한다. 화면에서 상태를 다시 계산하거나 기존 조건 입력을 읽지 않는다. 실제 정책·서버 API 연결은 없다. 웹 24건·린트·타입 검사·빌드, 개발 200·운영 404, 모바일·데스크톱과 예시 전환·근거 펼치기·새로고침 초기화를 확인했다. 서버 검사는 다시 실행하지 않았다.
...
- `npm run test:eligibility`: 소득 47건·취업 21건·거주 17건·연령 18건·집계 14건, 총 117건. 소득 비교기 작업에서 같은 테스트 범위와 서버 `assemble`을 한 Gradle 명령으로 실행해 모두 통과했다. API 인증키·DB·Docker가 필요하지 않다. 앞선 CI 구성 작업에서는 전체 서버 `build`의 당시 단위 14개·PostgreSQL 통합 2개가 모두 통과했다. 이번 결과 화면 작업에서 서버·PostgreSQL 통합 검사는 다시 실행하지 않았다.
...
- 다음은 모집 기간·마감 상태의 날짜 모델을 정리한다. 신청기간과 사업기간, 서울 기준 날짜·명시적 마감 시각, 상시·기간 미확인을 구분한다. 확인한 날짜만 비교하는 작은 서버 모델부터 검토하고 실제 원천 기간 문자열 파서는 성공 응답 확보 후 작성한다. 결과 화면의 모집 안내는 현재 고정 예시이며 모집 상태 계산·API 계약·실제 정책 연결은 아직 없다.
...
- 현재 검증 범위는 개발 환경·DB 연결·접근 차단, 비회원 조건 입력·인공 예시의 추가 질문과 결과 표시, 서버의 판정 결과 집계와 명시적 연령·거주·단일 취업·소득 구간 비교다. 실제 정책 판정 정확도, 외부 연동, 소셜 인증이나 알림 발송 검증으로 확대해서 보고하지 않는다.
