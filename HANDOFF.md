...
이번에는 같은 설계에 Flyway V1의 `ai_budgets`·`ai_request_reservations`와 `AiBudgetReservationStore`를 추가했다. 예산 행을 잠그고 최신 잔액·기간·최대 비용을 다시 확인한 뒤 예약 INSERT와 예약액 증가를 한 트랜잭션에서 처리한다. 예약 ID와 정책·AI 작업·요청 순번의 중복을 막고 동시 요청 두 건 중 하나만 저장함을 실제 PostgreSQL에서 확인했다. 전용 DB 5건·전체 서버 247건과 빌드가 통과했다. 첫 전용 실행에서 Java `Instant` SQL 유형 문제로 5건이 실패했으나 UTC `OffsetDateTime`·마이크로초 매핑 후 전용·전체가 통과했다. 실제 예산 배분·AI 호출·호출 이후 DB 상태·공급자 청구 검증은 없고 화면·API 계약은 변경하지 않았다.
...
- `npm run test:preview-api`: 마감 API 4개·자격 API 3개·인공 답변 재판정 5개·공통 명세 1개, 총 13개. 이번 전체 서버 빌드에서 도메인 227개와 PostgreSQL 예약·연결/기본 차단 7개도 함께 실행해 총 247개가 통과했다.
...
- 같은 패키지의 `AiBudgetReservationStore`: PostgreSQL 예산 행 잠금·최신 잔액 재검사·요청별 최초 예약을 처리한다. 외부 호출과 호출 이후 상태 변경은 수행하지 않는다.
...
- `npm run test:ai-reservation-db`: PostgreSQL 원자적 예약 5건. Flyway 테이블, 예약·잔액 동시 갱신, 중복·최신 잔액·한도·동시 요청·DB 제약을 확인한다. Docker가 필요하다.
...
- Flyway V1은 AI 예산·요청 예약 테이블만 만든다. 정책 원천·회원·일정·알림 테이블은 아직 없다.
- 빌드와 PostgreSQL 예약·연결/기본 차단 테스트 7개가 통과했다. 로컬 서버의 상태 응답 200과 차단 경로 403, 시작 화면은 앞선 작업에서 확인했다.
...
- 요청별 예산 예약·호출 결과 미확인·정산의 내부 상태와 중복 처리 기준을 구현했고, 최초 `HELD` 예약은 실제 PostgreSQL에서 원자성과 동시 요청을 검증했다. 실제 호출·호출 이후 상태·공급자 청구 확인은 아니다.
- 다음 독립 작업 후보는 외부 호출 식별자·결과 미확인·정산·호출 전 취소·확인된 무과금 해제의 PostgreSQL 상태 전이와 재시작 후 조회 기준이다. 실제 공급자 없이 내부 멱등 상태는 구현할 수 있지만 호출·청구 검증을 완료로 표시하지 않는다.
...
- 현재 검증 범위는 개발 환경·DB 연결·접근 차단, 비회원 입력·인공 예시 표시, 서버의 조건 비교·모집·후보 계산, 개발용 자격·마감 API와 생성 계약·화면 연결·실패 복구, 내부 개정 적용 판단·수집 진행·AI 후보 버전·사전 비용·메모리 예약 상태와 PostgreSQL 최초 예약 검사다. 실제 수집·정책 원본 저장·AI 외부 연동·호출 이후 정산·소셜 인증·알림 예약·발송·과금 차단 검증으로 확대해서 보고하지 않는다.
