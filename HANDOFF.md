...
이번에는 [AI 요청 예산 예약·정산 상태](docs/development/ai-budget-reservation-lifecycle.md)를 추가했다. 요청별 최대 비용을 보유하고 외부 호출·결과 미확인·정산·호출 전 취소·확인된 무과금 해제를 구분한다. 타임아웃은 예약액을 유지하며 실제 비용이 예약액을 넘으면 사실을 반영하고 초과 정산으로 남긴다. 동일 요청·예약·외부 결과의 재전달과 충돌도 구분한다. 전용 13건, 수집 모듈 51건과 전체 서버 242건·빌드가 실패·건너뛰기 없이 통과했다. 순수 메모리 모델이며 실제 PostgreSQL 원자적 예약·AI 호출·공급자 청구 검증·과금 차단은 없고 화면·API 계약·스키마도 변경하지 않았다.
...
- `npm run test:preview-api`: 마감 API 4개·자격 API 3개·인공 답변 재판정 5개·공통 명세 1개, 총 13개. 이번 전체 서버 빌드에서 도메인 227개와 실제 DB·기본 차단 2개도 함께 실행해 총 242개가 통과했다.
...
- 요청별 예산 예약·호출 결과 미확인·정산의 내부 상태와 중복 처리 기준도 구현했다. 실패를 무과금으로 처리하지 않지만 메모리 전이만 검증했으며 실제 DB 예약·공급자 청구 확인은 아니다.
- 다음 독립 작업 후보는 AI 요청 예약의 PostgreSQL 저장 구조·원자적 잔액 차감·재시작 후 소유권과 결과 미확인 조회 기준이다. 원천 성공 응답과 AI 공급자 계약이 없어도 내부 저장 경계는 설계할 수 있지만, 실제 호출·가격·청구 검증을 완료로 표시하지 않는다.
...
- 현재 검증 범위는 개발 환경·DB 연결·접근 차단, 비회원 입력·인공 예시 표시, 서버의 조건 비교·모집·후보 계산, 개발용 자격·마감 API와 생성 계약·화면 연결·실패 복구, 내부 개정 적용 판단·수집 진행·AI 후보 버전·사전 비용·메모리 예약 상태 검사다. 실제 수집·원본 저장·DB 중복 방지·정책 정확도·외부 연동·소셜 인증·예약·발송·과금 차단 검증으로 확대해서 보고하지 않는다.
