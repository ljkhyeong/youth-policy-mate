...
- 현재 `codex/policy-source-notices`의 `676fe5d`에서 정책 상세에 원문 충돌 안내를 추가했다. 청년내일저축계좌의 검토한 내용에 소득 기준 차이와 복지로 2026년 공고 링크를 표시한다. 원문·개정·자격 질문 제공 범위는 유지한다.
- 이번 검증의 명령·로그·실제 화면·제약은 [원문 충돌 안내](docs/development/policy-source-notices.md#검증)에 있다. 로컬 변경은 커밋했으며 이번 브랜치는 원격에 반영하지 않았다.
...
- 검증 기준: `1b7fbdd`의 전체 CI 이후 `676fe5d`의 상세 조회·충돌 안내를 정책 통합 테스트, 관련 웹 테스트, 명세 생성·타입·린트·웹 빌드·서버 패키징과 실제 화면으로 확인했다. 이후 변경은 문서뿐이다. DB는 V20이며 실제 정책 데이터는 변경하지 않았다.
...
1. 다른 정책의 원문·예외를 검토해 질문과 기본 조건 비교 범위를 늘린다. 청년내일저축계좌는 충돌 안내만 추가했다. 자격 질문은 원문 정합성과 지원금·참여 제한·증빙 기준을 더 검토한 뒤 연결한다. 원문이 바뀌면 기존 충돌 안내도 재검토한다.
