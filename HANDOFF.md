...
- `codex/condition-policy-discovery`의 `4c3044d`에서 관리자 실패 항목 재처리와 사유·작업자·결과 이력을 추가했다. 개인 조건 검색·정렬, 이사비 질문, 접수 상태 표시·필터, 청약통장 기본 연령 비교, 항목·페이지 실패 조회와 직전 개정 비교가 포함된 브랜치이며 아직 `main`에 병합하지 않았다.
...
- 관리자 수집 예외: `/admin/collection-exceptions`에서 원본·직전 개정 비교와 사유를 남기는 항목 재처리를 제공한다. 페이지 실패는 `/pages`, 관리자 재처리 이력은 `/replays` 하위 경로다. 재처리·시도·관리자 기록은 함께 커밋하며 중복 요청은 한 번만 반영한다. V19가 이력을 보관한다. 명시한 회원 UUID·소셜 세션·CSRF를 검사한다. 실제 소셜 계정 연결과 원본 값 보정은 남아 있다. [설정과 계약](docs/development/admin-collection-exceptions.md)
...
- 최근 검증: `4c3044d`의 관리자 재처리·동시 요청·롤백·권한 테스트, 서버 전체 검사·빌드, 웹 테스트·API 계약·최종 린트·타입·빌드를 통과했다. 브라우저에서 PC·모바일·키보드 실행·응답 유실 후 같은 ID 재전송·이력 1건·충돌 차단을 확인했다. 마지막 수정은 버튼 문구·투명도이며 웹 검사·빌드와 표시를 다시 확인했다. 로컬 V19 적용·정책 40건·비회원 차단을 확인했고 실제 정책 재처리·소셜 로그인은 실행하지 않았다. 이후 변경은 문서뿐이다. [명령·범위·결과](docs/development/admin-collection-exceptions.md#검증)를 참고한다.
...
3. 관리자 보정 값·근거·적용 개정을 원본과 분리해 기록하고, 새 수집과의 충돌을 처리하는 보정 기능을 구현한다.
