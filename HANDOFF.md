...
- `npm run test:preview-api`: 마감 API 4개·자격 API 3개·인공 답변 재판정 5개·공통 명세 1개, 총 13개. 이번 전체 서버 빌드에서 도메인 227개와 PostgreSQL 예약·실행·복구·연결/기본 차단 37개도 함께 실행해 총 277개가 통과했다.
...
- `npm run test:ingestion`: 수집 진행 12·AI 후보 12·사전 판단 14·예약 상태 13건, 총 51건. 부분 실패·명시적 종료·중단·재개·늦은 결과·AI 버전·재사용·비용·예약 상태를 확인한다. 인증키·DB·Docker 없이 실행한다. 실제 재시작·동시 작업자·중복 정책 반영·과금 차단 검증은 아니다.
...
- `npm run test:ai-recovery-execution`: PostgreSQL과 인공 복구 포트 9건. 트랜잭션 밖 확인, 정산·무과금·취소, 교체·만료된 임대와 변경된 예약의 결과 차단, 실패·수동 검토·재전달을 확인한다. Docker가 필요하다.
...
- 빌드와 PostgreSQL 예약·실행·복구·연결/기본 차단 테스트 37개가 통과했다. 로컬 서버의 상태 응답 200과 차단 경로 403, 시작 화면은 앞선 작업에서 확인했다.
...
- 다음 독립 작업 후보는 인공 실행·복구 결과를 현재 정책 개정과 비교해 `PolicyAiCandidateState`에 반영하는 것이다. 실제 후보 DB 저장은 정책·원본 저장 구조가 생긴 뒤 연결한다.
...
- 현재 검증 범위는 개발 환경·DB 연결·접근 차단, 비회원 입력·인공 예시 표시, 서버의 조건 비교·모집·후보 계산, 개발용 자격·마감 API와 생성 계약·화면 연결·실패 복구, 내부 개정 적용 판단·수집 진행·AI 후보 버전·사전 비용·메모리 예약 상태, PostgreSQL 예약·내부 상태 전이·인공 실행·복구 소유권과 펜싱이다. 실제 수집·정책 원본 저장·AI 외부 연동·공급자 청구 확인·소셜 인증·알림 예약·발송·과금 차단 검증으로 확대해서 보고하지 않는다.
