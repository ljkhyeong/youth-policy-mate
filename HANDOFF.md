...
- `npm run test:preview-api`: 마감 API 4개·자격 API 3개·인공 답변 재판정 5개·공통 명세 1개, 총 13개. 이번 전체 서버 빌드에서 DB 없는 도메인 246개와 PostgreSQL 예약·실행·복구·heartbeat·연결·운영 조회·수동 검토 재개·작업 배정·제한 목록·작업 실행 기록/운영 중단/실행-시도 연결/기본 차단 82개도 함께 실행해 총 341개가 통과했다.
...
- Flyway V1은 AI 예산·요청 예약 테이블, V2는 호출 이후 상태 열·제약·미완료 조회 인덱스, V3는 미완료 예약 복구 시도 이력, V4는 복구 작업 실행 ID·상태·결과 집계, V5는 운영 중단 사유·운영자와 상태 제약, V6는 작업 실행과 복구 시도 연결, V7은 수동 검토 재개 감사, V8은 활성 임대 갱신 감사를 만든다. 정책 원천·회원·일정·알림 테이블은 아직 없다.
- 빌드와 PostgreSQL 예약·실행·복구·임대 갱신·heartbeat·연결·운영 조회·수동 검토 재개·작업 배정·제한 목록·작업 실행 기록/운영 중단/실행-시도 연결/기본 차단 테스트 82개가 통과했다. 전체 서버 341건이며 로컬 서버의 상태 응답 200과 차단 경로 403, 시작 화면은 앞선 작업에서 확인했다.
...
- 활성 임대 갱신의 소유자·시도 순번·시각 펜싱과 감사 기록, 외부 확인 중 자동 heartbeat와 갱신 실패 뒤 결과 폐기를 구현했다. 다음 독립 작업 후보는 `HeartbeatStopped`를 기존 조정자 미완료 집계와 분리해 작업 실행 기록에 보존할지 결정하는 것이다. 실제 공급자 조회·운영 일정·고정 재확인 값은 공급자 선정 전까지 넣지 않는다.
