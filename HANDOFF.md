...
- `c8986df`까지 상반기 이사비 지원의 소득·중복지원 질문을 연결했다. 서울시 생애 1회, 타 기관 한쪽 비용 지원과 신청 비용, 일부 중복·미확인을 구분한다. 규칙은 `moving-fee-2026-h1-v3`이며 이전 답변은 차단한다. [검증 명령·로그·화면 확인](docs/development/moving-fee-questions.md#검증)을 통과했으며 이후 변경은 문서뿐이다.
...
- 조건 질문: 국가근로장학금·응시료 지원·K-패스·청년주택드림청약통장·서울청년정책네트워크·상반기 이사비 지원 6개 정책을 제공한다. 이사비는 보험료 기준 소득과 수혜 이력·신청 비용별 중복지원도 확인하며 증빙 심사는 별도다. 일부 요건만 비교하므로 전체 자격은 추가 확인 필요다. [질문 탐색](docs/development/policy-question-discovery.md)·[이사비 질문](docs/development/moving-fee-questions.md)
...
- 검증 기준: `152e331`의 보정·충돌·중복 요청·롤백·권한을 포함한 전체 서버 검사와 관련 웹 테스트·린트·타입·빌드가 통과했다. 마지막 성공 응답 계약 주석은 실제 OpenAPI 생성 검사·타입 검사·서버 패키징으로 확인했다. 로컬 V20·정책 40건·비회원 차단과 보정 화면은 [정책 보정 검증](docs/development/policy-corrections.md#검증)에 있다. 이후 로그인 통합 테스트를 추가했고, `c8986df`의 이사비 질문 변경은 관련 정책 규칙·API·패키징과 실제 화면으로 검증했다. 전체 검사는 반복하지 않았다. 웹 소스·DTO·DB 스키마와 실제 정책 데이터는 바꾸지 않았다.
