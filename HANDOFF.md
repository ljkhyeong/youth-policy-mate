...
서울 대상 MVP의 제품 범위와 기술 스택을 합의하고 기준 문서·전용 스킬, Next.js·Spring Boot·로컬 PostgreSQL 기본 구성을 작성했다. 시작 화면·비회원 조건 입력 화면과 서버 상태 확인을 제공한다. 서버에 자격 판정 결과 집계 모델을 추가했다. 정책 API, 업무 DB 스키마, 수집·실제 조건 비교·인증·저장·알림 기능은 아직 구현하지 않았다.

인증키 승인 전에도 개발을 진행하기로 했다. `/conditions`에서 생년월일·서울 자치구·주된 취업상태를 입력하고 확인·수정·초기화할 수 있다. 개인정보는 React 화면 상태로만 사용하며 서버 전송·브라우저 저장소 사용은 없다. 새로고침하면 초기화된다. 입력 검사와 정책 자격 판정은 별개이며, 실제 정책이나 가상 추천 결과는 표시하지 않는다.
...
- `npm run test:web`: Vitest 입력 검사 테스트 5개. 필수값, 날짜, 선택지, 연령 제한의 비적용과 서울 자정 경계를 확인한다.
- `backend/`: Java 25·Spring Boot·Spring MVC·JPA·Flyway·Spring Modulith core. GET `/actuator/health`만 허용하고 기본 로그인 계정을 생성하지 않는다.
- `backend/src/main/java/kr/youthpolicymate/eligibility/`: 순수 Java 판정 결과·근거 모델. 정책 검토 미완료·조건 미해석을 먼저 보류하고 명확한 불충족·사용자 정보 누락·전체 충족을 구분한다. 원문 해석·연령/지역/소득 비교·API/UI 연결은 없다.
- `npm run test:eligibility`: 판정 결과 집계만 검사하는 단위 테스트 14건 통과. 백엔드 `assemble` 빌드도 통과했다. API 인증키·DB·Docker가 필요하지 않다. 전체 서버 통합 테스트는 기존처럼 Docker가 필요하며 이번 집계 모델 작업에서는 다시 실행하지 않았다.
...
- 웹 테스트·린트·타입 검사·빌드와 서버 단위·통합 테스트의 CI 구성.
- 정책 목록·상세에 공통으로 쓸 로딩·빈 결과·오류 화면. 실제 정책으로 오인할 가상 추천은 제공하지 않는다.
...
- 현재 검증 범위는 개발 환경·DB 연결·접근 차단, 비회원 조건 입력과 서버의 판정 결과 집계다. 실제 정책 판정 정확도, 외부 연동, 소셜 인증이나 알림 발송 검증으로 확대해서 보고하지 않는다.
- 배포·CI·운영 인증 설정은 아직 없다. 로컬 DB 계정과 비밀번호를 운영 환경에 재사용하지 않는다.
