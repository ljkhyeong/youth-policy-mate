...
- `codex/condition-policy-discovery`의 `453a36e`에서 관리자 수집 실패 상세에 현재·직전 내부 개정 비교를 추가했다. 개인 조건 검색·정렬, 이사비 질문, 접수 상태 표시·필터, 청약통장 기본 연령 비교, 항목·페이지 수집 실패 조회가 포함된 브랜치이며 아직 `main`에 병합하지 않았다.
...
- 관리자 수집 예외: `/admin/collection-exceptions`에서 항목 실패·원본·현재 공개 내용과 직전 개정을 비교하고, `/admin/collection-exceptions/pages`에서 페이지 수집 실패를 조회한다. 개정의 원본 수집 시각을 제공하며 실패 당시 내용·개정 적용 시각으로 해석하지 않는다. 명시한 회원 UUID의 소셜 세션만 허용하고 기본 차단한다. 실제 소셜 계정 연결·보정·재처리 API는 아직 없다. [설정과 계약](docs/development/admin-collection-exceptions.md)
...
- 최근 검증: `453a36e`의 관리자·정책 저장소 테스트, API 계약·린트·타입 검사, 웹 배포 빌드·서버 패키징과 PC·모바일 개정 비교·키보드 펼치기를 통과했다. 전체 서버 검사는 관리자 읽기 경로에 한정된 변경이므로 반복하지 않았다. 이전 전체 검사의 기준은 `fe88cce`다. 최신 API로 로컬 서버를 재시작해 정책 40건과 관리자 비회원 차단을 확인했다. 이후 변경은 문서뿐이며 실제 관리자 소셜 로그인은 미검증이다. [명령·범위·결과](docs/development/admin-collection-exceptions.md#검증)를 참고한다.
...
3. 관리자 보정 근거·적용 개정을 기록하고 보정·재처리 API와 화면을 구현한다.
