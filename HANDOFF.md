...
- 현재 `codex/policy-source-notices`의 `6e4a2e5`에서 전세보증금반환보증 보증료 지원 질문을 추가했다. 보증 가입·납부, 보증금·본인과 배우자 무주택·유형별 소득을 비교한다. 소득서류 기준 미확인과 최종 서류·예산 심사를 구분한다.
- 이번 검증의 명령·로그·실제 화면·제약은 [보증료 지원 질문](docs/development/guarantee-fee-questions.md#검증)에 있다. 로컬 변경은 커밋했으며 이번 브랜치는 원격에 반영하지 않았다.
...
- 조건 질문: 국가근로장학금·응시료 지원·K-패스·청년주택드림청약통장·서울청년정책네트워크·상반기 이사비 지원·청년내일저축계좌·전세보증금반환보증 보증료 지원을 제공한다. 정책별 소득·중복지원·참여 제한의 미확인을 구분한다. 실제 증빙·기타 제한·선발은 기관 심사가 필요하며 전체 자격은 추가 확인으로 유지한다. [질문 탐색](docs/development/policy-question-discovery.md)·[이사비](docs/development/moving-fee-questions.md)·[저축계좌](docs/development/youth-tomorrow-savings-questions.md)·[보증료](docs/development/guarantee-fee-questions.md)
...
- 개인 조건 탐색: 저축계좌를 포함한 6개 정책의 연령을 비교하고 전체 정책에서 검색·정렬한다. 청약통장은 오늘 가입 기준의 나이와 병역기간 차감 미확인, 저축계좌는 2026년 5월 모집의 출생일 범위를 적용한다. 거주·취업·소득은 추가 확인으로 남긴다. 검색·페이지 이동·오류 재시도·모바일 화면을 로컬 실제 정책으로 확인했다.
...
- 검증 기준: `1b7fbdd`의 전체 CI 이후 상세 충돌 안내, 저축계좌 질문·기본 연령 비교와 보증료 질문을 추가했다. `6e4a2e5`의 보증료 규칙·PostgreSQL 정책 API·서버 패키징과 실제 화면을 확인했다. API 구조·생성 타입·웹 코드는 `676fe5d` 검증 이후 같아 해당 웹 검사를 재사용한다. 이후 변경은 문서뿐이며 DB V20·실제 정책 데이터는 유지했다.
...
1. 다른 정책의 원문·예외를 검토해 질문과 기본 조건 비교 범위를 늘린다. 보증료는 주소·명의·지원 제외·증빙·예산·지급액, 저축계좌는 제외업종·신용정보·증빙·선정 심사와 가입 후 유지·지급 조건이 남아 있다. 원문이 바뀌면 질문과 충돌 안내를 재검토한다.
