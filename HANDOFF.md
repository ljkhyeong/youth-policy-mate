...
- `npm run test:ai-recovery`: PostgreSQL 복구 소유권·수동 검토 재개 11건. 직접·다음 대상 획득, 동시 작업자, 재전달·충돌, 임대 만료, 완료 이력과 종료 예약 차단, 재개 감사·예약 스냅샷·최신 시도 펜싱·동시 재개·DB 사유 제약을 확인한다. Docker가 필요하다.
...
- Flyway V1은 AI 예산·요청 예약 테이블, V2는 호출 이후 상태 열·제약·미완료 조회 인덱스, V3는 미완료 예약 복구 시도 이력, V4는 복구 작업 실행 ID·상태·결과 집계, V5는 운영 중단 사유·운영자와 상태 제약, V6는 작업 실행과 복구 시도 연결, V7은 수동 검토 재개 감사를 만든다. 정책 원천·회원·일정·알림 테이블은 아직 없다.
- 빌드와 PostgreSQL 예약·실행·복구·연결·운영 조회·수동 검토 재개·작업 배정·제한 목록·작업 실행 기록/운영 중단/실행-시도 연결/기본 차단 테스트 75개가 통과했다. 전체 서버 334건이며 로컬 서버의 상태 응답 200과 차단 경로 403, 시작 화면은 앞선 작업에서 확인했다.
...
- 다음 독립 작업 후보는 실제 확인 시간이 임대보다 길어질 때 필요한 활성 임대 갱신의 소유자·시각 펜싱 계약을 정하는 것이다. 실제 공급자 조회·운영 일정·고정 재확인 값은 공급자 선정 전까지 넣지 않는다.
...
- 현재 검증 범위는 개발 환경·DB 연결·접근 차단, 비회원 입력·인공 예시 표시, 서버의 조건 비교·모집·후보 계산, 개발용 자격·마감 API와 생성 계약·화면 연결·실패 복구, 내부 개정 적용 판단·수집 진행·AI 후보 버전·사전 비용·메모리 예약 상태, PostgreSQL 예약·내부 상태 전이·인공 실행·복구 소유권·펜싱·읽기 전용 운영 조회·수동 검토 재개 감사·작업 배정·배정 실행·제한 목록 실행·작업 실행 기록·오래된 실행 조회와 운영 중단·실행과 복구 시도의 직접 연결, 실행·복구 응답의 현재 후보 재검사다. 실제 수집·정책 원본 저장·AI 외부 연동·후보 DB 저장·공급자 청구 확인·관리자 인증·소셜 인증·알림 예약·발송·과금 차단 검증으로 확대해서 보고하지 않는다.
