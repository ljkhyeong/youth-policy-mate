...
소득 설계에 따라 정의·대상·기간·적용 기준이 같은 답변의 원화 구간을 비교하는 순수 Java 모델을 구현했다. 사용자 구간 전체가 허용 범위 안이면 충족, 전혀 겹치지 않으면 불충족, 일부만 겹치면 사용자 정보 부족이다. 정책·개정·가구·기준표·기준일이 다른 답변은 재사용하지 않는다. 소득 47건을 포함한 판정 테스트 117건과 서버 `assemble`이 통과했다. 소득 화면·원천 매핑·단위 변환·기준표 조회·보험료 산정은 없으며 웹·PostgreSQL 통합 검사는 이번 작업에서 다시 실행하지 않았다.
...
- `npm run test:web`: Vitest 테스트 12개. 기존 입력 검사 5개, 상태 안내·오류 원문 비노출·404 복귀 경로 3개, 추가 질문·답변 표시 4개를 확인한다. 질문 화면 작업에서 테스트·린트·타입 검사·프로덕션 빌드를 통과했다.
...
- `npm run test:eligibility`: 소득 47건·취업 21건·거주 17건·연령 18건·집계 14건, 총 117건. 이번에는 같은 테스트 범위와 서버 `assemble`을 한 Gradle 명령으로 실행해 모두 통과했다. API 인증키·DB·Docker가 필요하지 않다. 앞선 CI 구성 작업에서는 전체 서버 `build`의 당시 단위 14개·PostgreSQL 통합 2개가 모두 통과했다. 소득 비교 작업에서 웹·PostgreSQL 통합 검사는 다시 실행하지 않았다.
...
- 다음은 개발 전용 소득 질문 미리보기에서 정의·대상 기간·원화 단위, 구간·모름·추가 확인 안내를 점검할 수 있다. 인공 예시임을 표시하고 실제 정책 판정·저장·공식 소득 확인으로 안내하지 않는다. 서버 비교기를 Next.js에 복사하지 않는다. 기준표 계산·월연 환산·원천 코드 매핑과 실제 정책 연결은 분리한다.
...
- 현재 검증 범위는 개발 환경·DB 연결·접근 차단, 비회원 조건 입력·인공 예시의 추가 질문, 서버의 판정 결과 집계와 명시적 연령·거주·단일 취업·소득 구간 비교다. 실제 정책 판정 정확도, 외부 연동, 소셜 인증이나 알림 발송 검증으로 확대해서 보고하지 않는다.
