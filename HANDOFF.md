...
- `be4a912`에서 실제 HTTP 기반 OAuth 로그인·JDBC 세션·관리자 권한·로그아웃 통합 테스트를 추가했다. 테스트와 실행 스크립트만 바뀌었고 운영 코드·화면은 같다. [로그인 검증](docs/development/member-policy-flow.md#oauth-전체-흐름-검증)의 명령을 통과했으며 이후 변경은 문서뿐이다.
- 정책 보정의 검증 명령·로그·실행 범위는 [정책 보정 검증](docs/development/policy-corrections.md#검증)에 있다. 이후 운영 코드·화면은 같고 로그인 통합 테스트와 문서만 추가했다. 기존 조건 비교 범위·개인정보 처리는 [개인 조건 탐색](docs/development/condition-policy-discovery.md)을 참고한다.
...
- 최근 검증: `152e331`의 보정·충돌·중복 요청·롤백·권한을 포함한 전체 서버 검사와 관련 웹 테스트·린트·타입·빌드가 통과했다. 마지막 성공 응답 계약 주석은 실제 OpenAPI 생성 검사·타입 검사·서버 패키징으로 확인했다. 브라우저에서 보정 생성·충돌 유지의 응답 유실 재확인, 중복 이력 없음, 409 차단, PC·모바일 표시를 확인했다. 로컬 V20·정책 40건·비회원 차단을 확인했으며 실제 정책 수정·소셜 로그인은 실행하지 않았다. 이후 운영 코드·화면은 같고 로그인 통합 테스트와 문서만 추가했다. [명령·범위·결과](docs/development/policy-corrections.md#검증)
