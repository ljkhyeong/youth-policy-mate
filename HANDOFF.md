...
- `43b85cd`에서 내 정책의 탭·일정 필터·이메일 설정 펼침 상태를 주소로 복원하도록 연결했다. 사용할 수 없는 수신 해제 링크는 이메일 설정을 바로 열고, 비회원·로그인 만료·제공자 로그인 실패 뒤에도 원래 화면으로 돌아간다.
- 웹 검사·화면 주소/로그인 복귀/회원 목록 테스트·배포용 웹 빌드를 통과했다. 헤드리스 브라우저에서 설정 바로가기·뒤로/앞으로 가기·새로고침·새 탭·모의 로그인 복귀·조회 오류·늦은 응답·모바일 키보드를 확인했다. 탭 이동에 따른 회원 목록 재조회는 없었다. [명령·범위·로그](docs/development/member-navigation.md#검증)
...
- 검증 기준: 현재 웹 코드는 `43b85cd`에서 웹 검사·관련 테스트·헤드리스 검증·배포용 웹 빌드를 통과했다. 전체 서버·웹·생성 계약은 기준 `main`의 `24c924c`에서 원격 CI를 통과했다. 이후 서버·계약은 변경하지 않아 재사용했다. 조건 입력·질문·회원 변경 요청·관리자 처리는 해당 코드가 같아 기존 검증을 유지했다. 이후 문서만 변경했으며 미커밋 코드는 없다. 실제 bfcache 저장·복원 전체 과정·공급자 송수신·홈서버 이미지 실행·실제 검색 색인과 작업 브랜치의 원격 CI는 미검증이다.
...
내 정책의 화면 복원과 이메일 설정 바로가기·로그인 복귀를 완료했다. 알림 목록 내부의 페이지·안 읽은 필터는 기존처럼 화면 상태로 관리하며 이번 주소 복원 범위에는 포함하지 않았다.
