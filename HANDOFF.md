...
- 소득 조건의 입력 의미와 비교 전제를 정리한다. 개인·가구 기준, 월·연 단위, 적용 연도·가구원 수를 구분하고, 실제 원문 확인 전 임의의 금액표나 원천 코드 매핑을 확정하지 않는다. 이미 만든 취업 질문 미리보기는 실제 정책·서버 연결과 구분한다.
...
- 현재 검증 범위는 개발 환경·DB 연결·접근 차단, 비회원 조건 입력·인공 예시의 추가 질문, 서버의 판정 결과 집계와 명시적 연령·거주·단일 취업 조건 비교다. 실제 정책 판정 정확도, 외부 연동, 소셜 인증이나 알림 발송 검증으로 확대해서 보고하지 않는다.
