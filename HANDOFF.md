...
- `codex/condition-policy-discovery`의 `df50b0f`에서 관리자 수집 예외 조회 API를 추가했다. 개인 조건 검색·정렬, 이사비 질문, 접수 상태 표시·필터, 청약통장 기본 연령 비교가 포함된 브랜치이며 아직 `main`에 병합하지 않았다.
...
- 관리자 수집 예외: 실패 항목 목록·저장 원본·동일 정책번호의 현재 내용을 조회한다. 명시한 회원 UUID의 소셜 세션만 허용하고 기본 차단한다. 기존 정책·원본·처리 이력은 변경하지 않는다. 관리자 화면·실제 계정 연결·보정·재처리 API는 아직 없다. [설정과 계약](docs/development/admin-collection-exceptions.md)
...
- 최근 검증: `df50b0f`의 관리자 PostgreSQL·권한 검사, 최종 전체 서버 테스트·빌드, 생성 API 타입·웹 린트·타입 검사를 통과했다. 로컬 서버에 반영해 정책 40건과 비회원 관리자 목록·상세 차단을 확인했다. 실제 관리자 로그인은 미검증이다. 화면 코드 변경이 없어 웹 빌드·브라우저 검사는 반복하지 않았다. [명령·범위·결과](docs/development/admin-collection-exceptions.md#검증)를 참고한다.
...
3. 관리자 화면·페이지 요청 실패 조회·이전 개정 비교·근거를 남기는 보정과 재처리를 구현한다.
