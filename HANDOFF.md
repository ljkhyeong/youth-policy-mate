...
- `8d650c9`에서 생년월일 자동 입력 중 빈 답변으로 조건 비교를 실행하는 문제를 수정했다. 진행 상태와 직접 입력 전환을 안내하고, 비교 중복 요청을 막으며 질문 시작·재조회·개정 변경 후 키보드 초점을 복구한다.
- 웹 검사·결과 표시/생년월일 메모리/API 중계 테스트·배포 빌드를 통과했다. 헤드리스 브라우저에서 수정 전 문제를 재현하고 자동 입력·비교 순서·직접 수정/삭제·실패·개정 변경·늦은 응답·키보드·모바일을 확인했다. [명령·범위·로그](docs/development/policy-rule-data.md#질문-화면-검증)
- 기존 3000 포트 개발 서버의 임시 검증 화면에서 실제 질문 컴포넌트를 확인했고, 검증 후 화면을 제거했다. 조건 API는 모의 응답으로 처리했고 회원 변경 요청은 없었으며 전용 브라우저를 종료했다. 서버·DB·운영 환경·사용자 창·탭은 변경하지 않았다. 외부 호출·이미지 빌드는 실행하지 않았다.
...
- 검증 기준: 웹의 현재 코드·배포 빌드 기준은 `8d650c9`다. 전체 서버·웹·생성 계약은 기준 `main`의 `24c924c`에서 원격 CI를 통과했다. 이후 서버·계약은 변경하지 않아 재사용했다. 질문 화면은 관련 테스트·웹 검사·헤드리스 검증·빌드로 확인했고 회원·관리자 요청 처리의 기존 검증도 해당 코드가 같아 유지했다. 이후 문서만 변경했으며 미커밋 코드는 없다. 실제 공급자 송수신·홈서버 이미지 실행·실제 검색 색인과 작업 브랜치의 원격 CI는 미검증이다.
...
회원·관리자 화면에 이어 조건 질문에서 확인한 자동 입력·비교 요청 충돌과 초점 유실을 처리했다.
