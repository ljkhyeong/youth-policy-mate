...
- `42c576a`에서 배포·외부 연동 코드를 추가 점검했다. Dockerfile·`.dockerignore`·운영 환경변수 예시, 운영 프로필·상태 확인·OAuth 경로, Resend 발송/웹훅/대기 처리, 웹의 실행 시 공개 주소·robots·사이트맵·회원 중계를 확인했고 추가 수정할 오류를 찾지 못했다. 해당 파일은 `git diff --name-only 24c924c -- <대상 파일>`에서 차이가 없었다. 코드 변경이 없어 테스트·빌드를 반복하지 않았다. [Resend 공식 문서](https://resend.com/docs/api-reference/rate-limit)의 기본 한도는 현재 팀당 초당 10회이며 앱은 10초마다 최대 5건을 처리한다. 실제 계정 한도·공급자 송수신·홈서버 이미지 실행은 별도 확인이 필요하다.
- `0761490`에서 수신 해제 실패·사용 불가 안내와 정책 변경 재조회 실패·성공 후의 키보드 초점 유실을 수정했다. 수신 해제 응답 유실은 결과 미확인으로 안내하고, 비교를 접으면 초점 이동 예약도 지운다.
- 웹 검사·중계/회원 목록 테스트를 통과했다. 헤드리스 브라우저에서 수정 전 네 가지 초점 유실을 재현하고 키보드 재시도·다음 행동 접근·링크 변경·늦은 응답·중복 방지·모바일을 확인했다. [명령·범위·로그](docs/development/email-unsubscribe.md#키보드와-오류-복구-검증)
- 기존 3000 포트 개발 서버의 수신 해제·`/my` 화면에서 회원 API를 모의 처리했고 실제 수신 해제·회원 변경·외부 호출은 없었다. 임시 화면을 추가하지 않았으며 전용 브라우저를 종료했다. 서버·DB·운영 환경·사용자 창·탭은 변경하지 않았다. 이미지 빌드는 실행하지 않았다.
...
- 검증 기준: 현재 웹 코드는 `0761490`에서 웹 검사·관련 테스트·헤드리스 검증을 통과했다. 마지막 배포 빌드는 `8d650c9`이며 이번 클라이언트 초점·문구 수정에서는 반복하지 않았다. 전체 서버·웹·생성 계약은 기준 `main`의 `24c924c`에서 원격 CI를 통과했다. 이후 서버·계약은 변경하지 않아 재사용했다. 조건 입력·질문·다른 회원 화면·관리자 요청 처리는 해당 코드가 같아 기존 검증을 유지했다. 이후 문서만 변경했으며 미커밋 코드는 없다. 실제 bfcache 저장·복원 전체 과정·공급자 송수신·홈서버 이미지 실행·실제 검색 색인과 작업 브랜치의 원격 CI는 미검증이다.
...
계정 전환의 초기화 누락에 이어 수신 해제·정책 변경 재조회에서 확인한 키보드 초점 유실을 처리했다.
