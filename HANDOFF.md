...
- `1547da6`에서 관리자 이메일 발송 기록에 Resend 상태 조회를 추가했다. 발송 ID가 있는 요청만 조회하며 공급자 이벤트와 조회 시각을 별도로 표시한다. 선택 환경변수 `RESEND_READ_API_KEY`의 빈 예시를 추가했고, 발송·DB 상태·수신 동의는 변경하지 않는다.
- `ce62bd4`에서 기존 Resend 발송 모듈에도 필요한 REST 클라이언트 의존성 누락을 해결했다. 실제 Resend 모듈의 운영 프로필 구성을 포함해 서버 전체 테스트·빌드, API 생성/계약·관련 웹 테스트·웹 검사·배포용 웹 빌드를 통과했다. 헤드리스에서 조회·오류/권한·중복 차단·키보드·모바일·늦은 응답을 확인했다. [명령·범위·로그](docs/development/email-provider-status.md#검증)
- 임시 PostgreSQL과 모의 API에 연결한 별도 웹을 사용했다. 실제 회원 변경·외부 공급자 호출은 없었고 검증용 브라우저·웹·API를 종료했다. 기존 3000 포트 서버·운영 환경·사용자 창·탭은 변경하지 않았다. 이미지 빌드는 실행하지 않았다.
...
- 검증 기준: `1547da6`까지 전체 서버 테스트·빌드, 생성 API 계약·회원 중계·관리자 이메일 화면 검사, 최종 웹 검사·빌드와 헤드리스 검증을 통과했다. 원격 CI 기준은 `main`의 `24c924c`다. 저장 정책 검색·모두 읽음·조건 입력·질문·이메일/계정 변경의 화면 코드는 같아 기존 브라우저 검증을 유지했다. 이후 문서만 변경했으며 미커밋 코드는 없다. 실제 bfcache 저장·복원 전체 과정·공급자 송수신/상태 조회·홈서버 이미지 실행·실제 검색 색인과 작업 브랜치의 원격 CI는 미검증이다.
...
서비스 알림의 모두 읽음에 이어 관리자 Resend 상태 조회와 운영 REST 클라이언트 구성을 완료했다.
