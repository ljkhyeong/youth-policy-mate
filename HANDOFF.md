...
- 현재 `codex/policy-source-notices`의 `37cd9f3`에서 수집 공고 개정·원본에 AI 추출 요청과 결과 본문을 연결하고 유효한 최신 결과를 규칙 초안으로 저장했다. `ai:policy-rules`의 원문 내보내기·결과 저장·상태 조회를 제공한다. 원문/요청 최신성·동일 결과 재시도·본문 보존·초안과 후보의 동시 롤백을 적용했다. 기존 관리자 화면에서 조회·편집·검토 후 적용한다. 실제 AI 호출이나 기존 예산 조정자와의 실행 연결은 아직 없다. 실제 11개 규칙은 변경하지 않았다.
- 새 AI 초안 통합 테스트와 전체 서버·DB·계약 검사를 통과했다. 명령·로그·미실행 범위는 [AI 규칙 초안 저장](docs/development/ai-rule-drafts.md#검증)에 있다. 화면·HTTP 계약·생성 타입을 바꾸지 않아 웹 검증은 `e05fa3d` 결과를 유지한다. 검증 후 변경은 문서뿐이다.
- 로컬 DB에 V26을 적용했다. 이전 확인 정책 40건·적용 규칙 11건은 변경하지 않았으며 AI 요청/후보를 로컬 실제 데이터로 만들지 않았다. Spring은 `37cd9f3` 실행 파일·PID 93917·로그 `/tmp/youth-ai-rule-runtime.log`, Next는 PID 70721이다. AI 운영 명령의 빈 결과 조회와 서버 정상 기동·상태 조회 200을 확인했다. 실행 시점에 프로세스를 재확인한다. 이번 브랜치는 원격에 반영하지 않았다.
...
- AI: 요청 원문·추출 본문·처리 상태·규칙 초안의 DB 저장을 연결했다. 후보 개정 검사·비용 예약·DB 복구 흐름은 별도 내부 모델과 테스트용 공급자로 검증했다. 실제 공급자 호출과 새 요청/본문 저장 경계를 기존 예산 실행·복구 경로에 연결하는 작업은 남아 있다. [규칙 초안](docs/development/ai-rule-drafts.md)·[요청 판단](docs/development/ai-request-admission.md)·[복구 실행](docs/development/ai-reservation-recovery-work-runs.md)
- 검증 기준: 전체 서버·DB·계약 검사는 `37cd9f3`, 관리자 편집 화면·타입·모바일 검사는 `e05fa3d`다. 이번 작업은 화면과 API 계약을 유지해 웹 검사를 반복하지 않았다. 원격 CI·실제 관리자 로그인·실제 AI 품질/청구는 미검증이다.
...
1. AI 공급자·모델·비용 한도를 정하고 실제 호출을 기존 예산 예약·실행·복구 조정자와 새 요청/본문 저장 경로에 연결한다. 호출 직전에도 원문·요청 최신성을 확인하고 수집 완료 후 자동 실행을 연결한다. 현재는 원문 내보내기와 확보한 결과 파일의 초안 저장까지 제공한다. 이전 공고의 연도만 바꾸거나 AI 결과를 바로 확정하지 않는다. 거주·취업·소득 재사용은 의미·기준일을 맞춘 뒤 확장한다.
