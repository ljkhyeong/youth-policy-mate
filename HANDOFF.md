...
- 현재 `codex/policy-source-notices`의 `b533da9`에서 기존 11개 정책의 질문·판정을 모두 공고 데이터로 통합했다. 정책별 운영 클래스와 질문 분기를 제거했고, 이전 구현은 고정된 비교 테스트 자료로 유지한다. 복합 선택지·고정 출생일·만 나이·월별 기준·모집 안내를 지원한다.
- 전체 서버 검사 556건·기존 OpenAPI 계약 일치·웹 검사와 실제 K-패스 연령 재사용·기준 월·첫 가입 월 예외, 청년주택드림청약통장 병역 예외 확인 대기·모바일 표시를 통과했다. 검증한 구현은 `b533da9`이며 이후 변경은 문서뿐이다. 명령·로그·미실행 범위는 [공고별 조건 데이터](docs/development/policy-rule-data.md#검증)에 있다. 문서 커밋 때문에 앱 검사를 반복하지 않는다.
- 로컬 DB는 V24·정책 40건·적용 규칙 11건이다. Spring은 PID 35300, 로그 `/tmp/youth-rule-expansion-backend.log`, Next는 PID 70721로 확인했다. 실행 시점에 재확인한다. 이번 브랜치는 원격에 반영하지 않았다.
...
- 검증 기준: 최신 전체 서버·DB·계약 검증은 `b533da9` 구현 기준이다. 공개 API 경로·DTO·생성 타입·의존성은 유지했다. 화면 변경은 재사용 안내 문구뿐이며 웹 검사를 통과했다. 원격 CI와 웹 빌드는 이번 작업에서 재실행하지 않았다.
...
- 이번 작업에서 운영 코드 일괄 치환과 검증을 묶은 명령은 자동 승인 검토에서 거절됐다. 파일별 변경 확인 → 컴파일 → 규칙·DB 검사 → 전체 검사로 나눠 완료했다. 같은 대량 치환 명령을 반복하지 않는다.
