...
- `b92d2cc`에서 서비스 알림의 ‘모두 읽음’을 추가했다. 모든 페이지의 본인 미읽음을 한 번에 처리하고 갱신 중 도착한 알림과 기존 읽은 시각을 유지한다. 개별 읽음의 오류 복구·페이지 복원 흐름을 공유한다.
- PostgreSQL 회원 통합 검사·API 생성/계약 검사·회원 중계 테스트·웹 검사·웹/서버 실행 파일 빌드를 통과했다. 헤드리스 브라우저에서 모두 읽음·개별 읽음·신규 미읽음·중복 요청 차단·응답 유실 복구·로그인 만료·페이지/필터·키보드·모바일을 확인했다. [명령·범위·로그](docs/development/member-notifications.md#검증)
- 기존 3000 포트 개발 서버와 모의 회원 응답, 임시 PostgreSQL을 사용했다. 실제 회원 변경·외부 공급자 호출은 없었고 전용 브라우저를 종료했다. 운영 환경·사용자 창·탭은 변경하지 않았다. 이미지 빌드는 실행하지 않았다.
...
- 검증 기준: `b92d2cc`에서 회원 통합 검사·회원 중계·생성 API 계약·웹 검사·헤드리스 검증·웹/서버 실행 파일 빌드를 통과했다. 전체 서버·웹·생성 계약의 원격 CI 기준은 `main`의 `24c924c`다. 저장 정책 검색(`bae7b16`)과 조건 입력·질문·이메일/계정 변경·관리자 화면은 코드가 같아 기존 검증을 유지했다. 이후 문서만 변경했으며 미커밋 코드는 없다. 실제 bfcache 저장·복원 전체 과정·공급자 송수신·홈서버 이미지 실행·실제 검색 색인과 작업 브랜치의 원격 CI는 미검증이다.
...
내 정책의 화면 복원·저장 정책 검색·변경 필터에 이어 서비스 알림의 모두 읽음을 완료했다.
