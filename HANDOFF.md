...
- `npm run test:preview-api`: 마감 API 4개·자격 API 3개·인공 답변 재판정 5개·공통 명세 1개, 총 13개. 이번 전체 서버 빌드에서 DB 없는 도메인 245개와 PostgreSQL 예약·실행·복구·연결·운영 조회·작업 배정·제한 목록·작업 실행 기록/운영 중단/기본 차단 67개도 함께 실행해 총 325개가 통과했다.
...
- 같은 패키지의 `AiReservationRecoveryWorkRunStore`·`AiReservationRecoveryWorkRunCoordinator`: 실행 ID·작업자·조회 조건과 시작/완료/실패, 후보별 결과 집계를 PostgreSQL에 저장한다. 오래된 실행 중 기록은 호출 측 기준으로 제한 조회하고 운영자가 선택한 경우에만 감사 정보와 함께 중단한다. 자동 종료와 주기 스케줄러는 없다.
...
- `npm run test:ai-recovery-work`: PostgreSQL 제한 목록 4건, 작업 실행 기록 6건, 오래된 실행 조회·운영 중단 5건, 총 15건. 보류·중단 후보 뒤의 준비된 후보, 조회 뒤 판단 변경, 조회 개수 제한, 후보별 외부 확인 실패 뒤 계속 실행, 실행 ID 재전달·충돌·동시 기동, 완료 집계·전체 실패, 오래된 실행 정렬·제한, 중단 감사 정보와 종료 상태 경합·DB 제약을 확인한다. Docker가 필요하다.
...
- Flyway V1은 AI 예산·요청 예약 테이블, V2는 호출 이후 상태 열·제약·미완료 조회 인덱스, V3는 미완료 예약 복구 시도 이력, V4는 복구 작업 실행 ID·상태·결과 집계, V5는 운영 중단 사유·운영자와 상태 제약을 만든다. 정책 원천·회원·일정·알림 테이블은 아직 없다.
- 빌드와 PostgreSQL 예약·실행·복구·연결·운영 조회·작업 배정·제한 목록·작업 실행 기록/운영 중단/기본 차단 테스트 67개가 통과했다. 전체 서버 325건이며 로컬 서버의 상태 응답 200과 차단 경로 403, 시작 화면은 앞선 작업에서 확인했다.
...
- 다음 독립 작업 후보는 복구 작업 실행과 예약별 시도 사이의 직접 연결 필요성을 정하는 것이다. 실제 공급자 조회·운영 일정·고정 재확인 값은 공급자 선정 전까지 넣지 않는다.
...
- 현재 검증 범위는 개발 환경·DB 연결·접근 차단, 비회원 입력·인공 예시 표시, 서버의 조건 비교·모집·후보 계산, 개발용 자격·마감 API와 생성 계약·화면 연결·실패 복구, 내부 개정 적용 판단·수집 진행·AI 후보 버전·사전 비용·메모리 예약 상태, PostgreSQL 예약·내부 상태 전이·인공 실행·복구 소유권·펜싱·읽기 전용 운영 조회·작업 배정·배정 실행·제한 목록 실행·작업 실행 기록·오래된 실행 조회와 운영 중단, 실행·복구 응답의 현재 후보 재검사다. 실제 수집·정책 원본 저장·AI 외부 연동·후보 DB 저장·공급자 청구 확인·소셜 인증·알림 예약·발송·과금 차단 검증으로 확대해서 보고하지 않는다.
