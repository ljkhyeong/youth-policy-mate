...
- 현재 `codex/policy-source-notices`의 `a6e807d`에서 내 정책 마감 일정을 개선했다. 공개 정책과 같은 접수 상태를 조회하고 가까운 마감부터 표시하며 접수 종료는 마지막에 둔다. `/my`에 접수 상태 필터·새로고침을 추가하고 상시 접수와 기간 미확인을 구분한다. 새로고침·저장 해제 후 필터를 유지한다.
- 회원·알림 PostgreSQL 통합 검사, 관련 웹 테스트, 계약 생성·타입·린트·배포 빌드를 통과했다. 조회는 기존 SELECT 세 번이며 서울 자정에 개정 변경 없이 접수 상태가 갱신되는 것을 확인했다. 390px·1280px 표시·키보드 저장 해제·필터 유지를 테스트 응답으로 확인했다. 마지막 모바일 클래스·스타일 수정은 빌드·브라우저 표시로 재확인했다. 명령·로그·제약은 [내 정책 마감 일정](docs/development/member-calendar.md#검증)에 있다. 최종 검증 후 변경은 문서뿐이다.
- 로컬 Spring은 `a6e807d` 실행 파일·PID 30454·로그 `/tmp/youth-member-calendar-runtime.log`, Next는 PID 10257·로그 `/tmp/youth-admin-ai-web.log`다. 상태 조회 200·회원 API 비회원 401·내 정책 화면 200을 확인했다. DB 마이그레이션은 V28을 유지했고 실제 회원 데이터를 변경하지 않았다. AI 키·모델·요금·한도는 미설정이다. 자동 추출·정기 수집·이메일·알림은 비활성화했고 실제 AI 호출은 실행하지 않았다. 실행 시점에 프로세스를 재확인하며 이번 브랜치는 원격에 반영하지 않았다.
...
- 회원 기능: OAuth 로그인 코드, 관심 정책 저장·해제, 일정·서비스 내 알림을 연결했다. [마감 일정](docs/development/member-calendar.md)에서 접수 상태를 구분하고 가까운 마감순 정렬·상태 필터·새로고침을 제공한다. 테스트 제공자와 실제 HTTP·DB로 코드 교환부터 관리자 접근·세션 종료까지 확인했다. 로컬 카카오·네이버 ID/Secret과 관리자 ID가 없어 실제 제공자 로그인은 미검증이다. [회원 기능](docs/development/member-policy-flow.md)
...
- 검증 기준: 회원·일정·알림 통합과 관련 웹·계약은 `a6e807d`, 관리자 API·웹은 `280a12b`, 변경하지 않은 자동 추출·생성·비용 처리 등 전체 서버 기준은 `ee34aca`다. 실제 제공자 로그인·AI 품질/청구·원격 CI는 미검증이다.
