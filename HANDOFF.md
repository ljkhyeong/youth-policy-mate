...
- `a6586c5`에서 로그인 방법 조회 재시도와 정책·관리자 복귀 경로를 보완했다. 일반 로그인은 이전 이동 기록을 지우고 제공자 실패 후 재시도는 직전 복귀 주소를 유지한다. 알림 읽음 처리의 중복·응답 유실·로그인 만료 복구와 모바일 메뉴의 버튼 가림도 수정했다.
- 웹 검사·로그인 복귀 테스트·운영 빌드를 통과했다. 헤드리스 브라우저에서 로그인 오류·복귀, 알림 읽음·페이지·응답 유실·401·초점·모바일을 확인했다. [로그인 검증](docs/development/member-policy-flow.md#로그인-화면-검증)·[알림 검증](docs/development/member-notifications.md#검증)
- 기존 3000 포트 개발 서버에서 확인했고 서버·DB·운영 환경은 변경하지 않았다. 로그인은 모의 제공자, 알림 변경 7건은 모의 API에서 처리했으며 검증 브라우저는 종료했다. 실제 회원 변경·외부 공급자 호출·이미지 빌드는 실행하지 않았다. 사용자 창·탭은 조작하지 않았다.
...
- 검증 기준: 웹의 현재 코드 기준은 `a6586c5`다. 전체 서버·웹·생성 계약은 기준 `main`의 `24c924c`에서 원격 CI를 통과했다. 이후 서버·계약은 변경하지 않아 재사용했고 로그인·알림·모바일 스크롤은 관련 검사와 브라우저로 확인했다. 조건·이메일 설정·대시보드·탈퇴의 기존 검증도 해당 동작 코드가 같아 유지했다. 이후 문서만 변경했으며 미커밋 코드는 없다. 실제 공급자 송수신·홈서버 이미지 실행·실제 검색 색인과 작업 브랜치의 원격 CI는 미검증이다.
...
정책 저장·회원 조건·이메일 설정·대시보드에 이어 로그인·알림 화면에서 확인한 오류 복구 누락을 처리했다. 모바일 하단 메뉴가 재시도 버튼을 가리는 문제도 수정했다.
