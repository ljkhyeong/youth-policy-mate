...
2026-09-07에 [관리자 수집 예외 조회 API](docs/development/admin-collection-exceptions.md)를 추가했습니다. 항목 검증·저장 실패 목록과 원본·현재 공개 내용을 확인합니다. 지정한 회원 ID의 소셜 로그인 세션만 접근할 수 있으며 기본 설정은 접근 차단입니다. 서버 전체 검사·빌드와 생성 타입·웹 검사를 통과했습니다. 관리자 화면·실제 계정 연결·원본 보정은 남아 있습니다.
