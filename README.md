...
2026-09-07에 [관리자 수집 예외 화면과 API](docs/development/admin-collection-exceptions.md)를 연결했습니다. ‘항목 처리’에서 원본과 현재·직전 개정의 변경 내용을, ‘페이지 수집’에서 실패 유형·HTTP 상태·요청 시각·응답 보관 여부를 확인합니다. 지정한 회원 ID만 접근하고 기본 설정은 차단입니다. 관련 서버·웹 테스트, 타입·빌드와 PC·모바일·키보드 조작을 검증했습니다. 실제 소셜 계정 연결·보정·재처리는 남아 있습니다.
