...
Flyway V1과 PostgreSQL 예약 저장소도 추가했습니다. 예산 행을 잠근 뒤 최신 잔액을 다시 확인하고 예약 INSERT와 예약액 증가를 한 트랜잭션에서 처리합니다. 예약 ID와 정책·작업·요청 순번 중복을 막고, 동시 요청은 하나만 예약합니다. PostgreSQL 전용 5건과 전체 서버 247건·빌드가 통과했습니다. 실제 AI 호출·호출 이후 DB 상태·공급자 청구 검증은 아직 없습니다.
