...
2026-09-08 기준 [개인 조건 기반 정책 탐색](docs/development/condition-policy-discovery.md)에서 청년내일저축계좌를 포함한 6개 정책의 연령을 비교하고 전체 정책에서 검색·정렬합니다. [전세보증금반환보증 보증료 지원 질문](docs/development/guarantee-fee-questions.md) 추가로 질문 제공 범위는 **8개 정책**입니다. 보증 가입·납부, 보증금·무주택, 유형별 소득을 확인하며 소득서류 확인 중 상태를 구분합니다. 최종 신청 자격과 접수·예산·실제 지급액은 추가 확인으로 남깁니다.
