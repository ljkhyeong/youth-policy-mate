...
2026-09-07에 [관리자 수집 예외 관리](docs/development/admin-collection-exceptions.md)를 연결했습니다. 실패 원본·개정 비교·페이지 실패를 조회하고, 사유를 입력해 항목 하나를 재처리합니다. 작업자·사유·결과·처리 후 개정은 별도 이력에 남고 중복 요청은 한 번만 반영합니다. 관리자 권한·CSRF, 서버 전체 검사·웹 테스트·빌드와 PC·모바일·응답 유실 흐름을 검증했습니다. 실제 소셜 계정 연결과 원본 값 보정은 남아 있습니다.
