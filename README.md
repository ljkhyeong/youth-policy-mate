...
2026-09-05에 [공통요건 질문이 있는 정책 찾기](docs/development/policy-question-discovery.md)를 추가했습니다. 목록의 질문 표시·필터·검색과 기본 조건 결과에서 상세 질문으로 이동할 수 있습니다. 현재 정책 40건 중 질문 제공 2건이며 원문 변경·적용 연도에 따라 서버가 제공 여부를 다시 확인합니다. 서버 422건·웹 66건과 계약·린트·타입·빌드, 실제 모바일·데스크톱·키보드 이동을 확인했습니다.

2026-09-05에 [청년 국가기술자격 응시료 지원 공통요건](docs/development/exam-fee-questions.md)을 추가했습니다. 공식 안내의 출생일 범위·대상 시험·큐넷 잔여 횟수를 비교하고 복구 대기·미확인은 추가 확인으로 남깁니다. 기존 국가근로장학금을 포함해 실제 **2개 정책의 공통요건 비교**를 제공합니다. 서버 전체 420건과 빌드가 통과했으며, 시험 응시자격·실시간 예산·결제 할인 확정은 포함하지 않습니다.
...
**현재 판정 범위는 두 정책의 공통요건입니다.** 국가근로장학금의 대학별 선발요건, 응시료 지원의 시험별 자격·현재 예산·결제 적용은 남아 있어 최종 신청 자격은 추가 확인 필요로 표시합니다. 다른 정책의 규칙 검토·개인별 추천 순위·이메일 공급자 실연동·정기 수집의 실제 운영 설정은 남은 작업입니다.
