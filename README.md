...
현재 제공하는 11개 정책의 질문·판정은 [공고별 조건 데이터](docs/development/policy-rule-data.md)로 관리합니다. 검토한 새 버전을 적용하면 서버 재시작 없이 질문과 조건 비교가 바뀝니다. [관리자 조건 검토·적용](docs/development/policy-rule-review.md)에서 원문 변경·미등록 정책을 찾고 기존 규칙 편집·초안 저장·검토 후 적용을 처리합니다. 파일 등록도 지원합니다. 확인한 생년월일은 9개 정책의 연령 질문에 재사용합니다. AI 초안 생성과 수집 결과 연결은 남아 있습니다.
