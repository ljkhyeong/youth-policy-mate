...
2026-09-05에 [조건 확인·회원 저장·서비스 내 알림](docs/development/member-policy-flow.md)을 추가했습니다. `/conditions`에서 실제 원문 요건을 확인하고, 설정된 소셜 로그인 후 기본 조건과 관심 정책을 저장할 수 있습니다. `/my`는 마감 일정·변경 알림·읽음 처리를 제공합니다. 서버 385건·웹 59건, API 계약·린트·타입 검사와 Webpack 빌드가 통과했습니다.

**아직 실제 자격 판정 완료 단계는 아닙니다.** 현재 정책은 원문 검토가 필요해 모두 추가 확인 필요로 표시합니다. 카카오·네이버 앱 키가 로컬에 없어 실제 소셜 로그인은 미확인이며, 이메일 발송·정기 전체 수집은 다음 작업입니다. 로그인 설정과 현재 제약은 위 실행 안내를 따릅니다.

2026-09-05에 **실제 정책 20건의 저장·검색·상세 조회**를 연결했습니다. `/policies`에서 제목·설명을 검색하고 지원 내용·신청 안내·공식 링크·수집 시각을 확인할 수 있습니다. 온통청년 서울 필터의 작은 표본이며 전체 정책 목록이나 개인별 자격 판정은 아닙니다. [조회 실행 방법](docs/development/policy-catalog.md)을 참고합니다.

Spring Batch와 PostgreSQL로 [한 페이지 수집·실행 이력·저장 원본 재처리](docs/development/limited-policy-collection.md)를 추가했습니다. `npm run collect:policy -- --args='fetch 2'`는 서울 필터의 지정 페이지를 최대 10건만 한 번 요청합니다. 실패 항목은 보관한 원본으로 재처리하며 정상 정책은 유지합니다. 이번 서버 전체 373건과 빌드가 통과했습니다. 정기 실행·전체 수집은 아직 켜지 않았습니다.
...
온통청년 인증키와 목록·상세 성공 응답을 확인했습니다. 실제 정책 조회는 제공하지만 자동 수집, 실제 정책을 연결한 자격 판정, 로그인·관심 정책 저장·알림은 아직 없습니다. 조건 입력과 비교기 계산은 실제 정책에 연결하지 않았습니다.
