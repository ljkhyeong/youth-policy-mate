...
현재 제공하는 11개 정책의 질문·판정은 [공고별 조건 데이터](docs/development/policy-rule-data.md)로 관리합니다. 검토한 새 버전을 적용하면 서버 재시작 없이 질문과 조건 비교가 바뀝니다. 원문 변경·기간 만료·미등록 정책은 [관리자 조건 검토](docs/development/policy-rule-review.md)에서 확인합니다. 확인한 생년월일은 9개 정책의 연령 질문에 재사용합니다. AI 초안 생성과 수집 결과 연결은 남아 있습니다.
