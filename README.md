...
2026-09-07에 [관리자 수집 예외 화면과 API](docs/development/admin-collection-exceptions.md)를 연결했습니다. ‘항목 처리’에서 원본·현재 공개 내용을, ‘페이지 수집’에서 실패 유형·HTTP 상태·요청 시각·응답 보관 여부를 확인합니다. 지정한 회원 ID만 접근하고 기본 설정은 차단입니다. 서버 전체 검사·웹 테스트·타입·빌드와 PC·모바일 흐름을 검증했습니다. 실제 소셜 계정 연결·이전 개정 비교·보정·재처리는 남아 있습니다.
