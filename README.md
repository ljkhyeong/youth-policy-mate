...
소득은 [입력 의미와 구간 비교 범위](docs/design/income-condition.md)를 설계했습니다. 개인·가구, 소득 정의·단위·기간이 맞는 값만 비교하고, 사용자 구간이 정책 경계에 걸치면 추가 확인으로 남깁니다. 소득 비교기·입력 화면·기준표 계산은 아직 구현하지 않았습니다.
...
온통청년 API 인증키는 신청 후 승인 대기 중입니다. 인증된 성공 응답, 정책 조회·수집, 실제 정책을 연결한 자격 판정, 로그인·저장·알림 기능은 아직 구현하지 않았습니다. 연령·거주·취업 비교기와 결과 집계 모델도 화면과 연결하지 않았습니다. 조건 입력 검사는 신청 자격 판정이 아니며 실제 정책이나 가상 추천 결과를 표시하지 않습니다.
