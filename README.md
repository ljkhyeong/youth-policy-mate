...
[AI 예약 복구 작업 실행 기록](docs/development/ai-reservation-recovery-work-runs.md)은 실행 ID·작업자·조회 조건을 Flyway V4 테이블에 먼저 저장하고, 같은 실행 ID의 동시·완료 재전달을 반복 실행하지 않습니다. Flyway V5는 강제 종료로 남은 오래된 `RUNNING`을 제한 조회하고 운영자가 선택한 경우에만 사유·운영자·시각과 함께 `ABORTED`로 중단하는 계약을 추가합니다. 완료·실패·중단 중 먼저 확정된 결과를 보존합니다. 제한 목록과 실행 기록·운영 중단 15건, 전체 서버 325건·빌드가 통과했습니다. 실제 공급자·주기 스케줄러·자동 중단과 관리자 API는 아직 없습니다.
