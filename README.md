...
2026-09-07에 [관리자 수집 예외 화면과 API](docs/development/admin-collection-exceptions.md)를 연결했습니다. `/admin/collection-exceptions`에서 실패 목록과 원본·현재 공개 내용을 확인하며 로그인 후 목록으로 돌아옵니다. 지정한 회원 ID만 접근하고 기본 설정은 차단입니다. 관련 웹 테스트·타입·빌드와 PC·모바일 흐름을 검증했습니다. 실제 소셜 계정 연결·페이지 요청 실패 조회·원본 보정은 남아 있습니다.
...
2026-09-07 기준 [개인 조건 기반 정책 탐색](docs/development/condition-policy-discovery.md)에서 청년주택드림청약통장을 포함한 5개 정책의 연령을 비교하고 전체 정책에서 검색·정렬합니다. 청약통장은 오늘 가입을 가정한 연령과 병역기간 차감 미확인을 구분합니다. [상반기 이사비 지원 질문](docs/development/moving-fee-questions.md) 추가로 질문 제공 범위는 **6개 정책**입니다. 연령 충족과 최종 신청 자격·접수 상태를 구분합니다.
...
**현재 조건 비교는 다섯 정책의 일부 요건을 제공합니다.** 국가근로장학금의 대학별 선발요건, 응시료 지원의 시험별 자격·현재 예산·결제 적용, K-패스의 실제 적립 인정·환급액·지급, 청년주택드림청약통장의 가입 서류·전환·혜택 심사, 서울청년정책네트워크의 서류·교육·퀴즈·종합 평가는 남아 있어 최종 신청 자격은 추가 확인 필요로 표시합니다. 다른 정책의 규칙 검토·개인별 추천 순위·이메일 공급자 실연동·정기 수집의 실제 운영 설정은 남은 작업입니다.
