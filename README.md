...
응시료 지원과 국가근로장학금은 [공고별 조건 데이터](docs/development/policy-rule-data.md)로 관리합니다. 검토한 새 버전을 적용하면 서버 재시작 없이 질문과 조건 비교가 바뀝니다. 원문 변경·기간 만료 시 재검토하며, 확인한 생년월일은 출생일 질문에 재사용합니다. 다른 정책 이전과 AI 초안 생성 연결은 남아 있습니다.
