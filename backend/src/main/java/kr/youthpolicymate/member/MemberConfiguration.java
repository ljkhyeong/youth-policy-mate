...
public class MemberConfiguration {
    ...
    @Bean
    Registrations clientRegistrationRepository(Environment env) {
    ...
    private void add(List<ClientRegistration> target, Environment env, String id, String name, String base,
        ...
        target.add(ClientRegistration.withRegistrationId(id).clientName(name).clientId(key).clientSecret(secret)
    ...
    public static class Registrations implements ClientRegistrationRepository, Iterable<ClientRegistration> {
        private final List<ClientRegistration> values;
        Registrations(List<ClientRegistration> values) { this.values = List.copyOf(values); }
        @Override public ClientRegistration findByRegistrationId(String id) {
            return values.stream().filter(value -> value.getRegistrationId().equals(id)).findFirst().orElse(null);
        }
        @Override public Iterator<ClientRegistration> iterator() { return values.iterator(); }
        public boolean enabled() { return !values.isEmpty(); }
    }
}
