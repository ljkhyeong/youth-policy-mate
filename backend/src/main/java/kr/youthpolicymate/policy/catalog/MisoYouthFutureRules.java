...
public final class MisoYouthFutureRules {
    ...
    private static final List<Option> CRITERION_OPTIONS = List.of(
            new Option("YES", "해당해요 · 확인 완료"), new Option("NO", "해당하지 않아요 · 확인 완료"),
    ...
    private static final List<Question> QUESTIONS = List.of(
            ...
            new Question("employment", "미소금융 지점에서 확인한 취업·창업 상태는 무엇인가요?",
                    "미취업 또는 취업·창업 후 1년 이내가 대상이에요. 근로·사업을 병행하거나 인정 기간을 모르면 ‘확인 중’을 선택해주세요.",
                    List.of(new Option("UNEMPLOYED", "미취업 · 지점 확인 완료"), new Option("EARLY_EMPLOYEE", "취업 1년 이내 · 지점 확인 완료"),
                            new Option("EARLY_BUSINESS", "창업 1년 이내 · 지점 확인 완료"), new Option("NOT_TARGET", "대상 아님 · 지점 확인 완료"),
                            new Option("PENDING", "취·창업 이력 확인 중"), new Option("UNKNOWN", "모르겠어요"))),
            new Question("credit", "개인신용평점 하위 20%에 해당하나요?",
                    "지점의 신용평가 기준으로 확인해주세요. 아래 신용평점·수급 자격·근로장려금 중 하나만 해당하면 돼요.", CRITERION_OPTIONS),
            new Question("welfare", "기초생활수급자 또는 차상위계층 이하에 해당하나요?",
                    "신청 시점의 수급·차상위 자격을 증빙서류로 확인해주세요.", CRITERION_OPTIONS),
            new Question("earnedIncomeCredit", "근로장려금 신청 자격 요건에 해당하나요?",
                    "적용 연도와 신청 자격·증빙서류를 미소금융 지점에서 확인해주세요. 과거 수령 이력만으로는 확인할 수 없어요.", CRITERION_OPTIONS));
    ...
    public static Questionnaire questionnaire(long revision) {
        return new Questionnaire(NUMBER, revision, VERSION, true, SCOPE,
                "연령·취업·창업 상태와 신용평점·수급 자격·근로장려금 요건을 확인해요. 최종 대출 여부는 지점 심사로 결정돼요.", SOURCE, QUESTIONS);
    }
    ...
    public static Evaluation evaluate(long revision, Request input, Instant now) {
        ...
        var checks = List.of(
                ...
                new Check("신용평점·수급·근로장려금 요건", "신용평점: " + provided("credit", values) + " / 수급·차상위: "
                        + provided("welfare", values) + " / 근로장려금: " + provided("earnedIncomeCredit", values), alternativeOutcome,
                        alternativeOutcome == MET ? "세 요건 중 하나 이상을 충족해요."
                                : alternativeOutcome == NOT_MET ? "세 요건 모두 비해당으로 확인됐어요."
                                : "아직 확인하지 못한 요건이 있어요. 세 요건 중 하나에 해당하는지 확인해주세요.",
        ...
        return new Evaluation(NUMBER, revision, VERSION, whole.status(), common, SCOPE,
                "입력한 기본 조건의 비교 결과예요. 지원 제한과 예외, 최종 대출심사는 미소금융 지점에서 확인해야 해요.",
