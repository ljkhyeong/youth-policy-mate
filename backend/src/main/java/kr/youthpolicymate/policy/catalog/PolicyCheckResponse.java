...
public record PolicyCheckResponse(List<Item> items, int page, long total, boolean hasNext, Instant evaluatedAt) {
    @Schema(name = "PolicyCheckItem", requiredProperties = {"policyNumber", "revision", "title", "status", "explanation", "applicationPeriod", "sourceUrl", "collectedAt", "checks", "questionnaireAvailable", "ruleVersion", "recruitment"})
    public record Item(String policyNumber, long revision, String title, EligibilityStatus status, String explanation,
                       ...
                       @Schema(description = "적용한 연령 규칙 버전. 검토된 비교 기준이 없으면 빈 문자열") String ruleVersion, PolicyRecruitment recruitment) {}
