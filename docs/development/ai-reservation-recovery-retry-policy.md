...
2026-09-01 구현 기준. [복구 소유권 설계](../design/ai-reservation-recovery.md)에 따라 미완료 AI 예약의 복구 이력으로 다음 확인 가능 시각과 자동 중단 사유를 계산한다. 실제 작업자·스케줄러·DB 대상 선택에는 연결하지 않았다.
...
순수 정책은 `claimNext`를 호출하거나 복구 이력을 수정하지 않는다. 실제 자동 작업자는 후보 예약과 이력을 읽고 이 정책이 `Ready`를 반환한 경우에만 소유권을 획득해야 한다. 판단 뒤 소유권 획득 사이의 경쟁은 기존 예약 잠금·활성 임대 제약으로 다시 확인한다.
...
`test:ingestion` 69건과 전체 서버 296건이 실패·오류·건너뛰기 없이 통과했다. 전체 구성은 DB 없는 도메인 245건, 개발 API·계약 13건, PostgreSQL 예약·실행·복구·연결/기본 차단 38건이다. 화면·API·Flyway 계약은 변경하지 않았다.

## 남은 작업

- 공급자 선정 뒤 실제 간격·최대 시도 횟수 확정
- 정책 결과를 적용하는 자동 작업자와 PostgreSQL 대상 선택
- 최대 횟수 도달·수동 검토 예약의 운영 조회와 명시적 재개
