...
`CHECK_FAILED`는 설정한 간격 뒤 재확인한다. `CHECK_COMPLETED`도 예약이 미완료이면 청구 대기 가능성이 있으므로 같은 규칙을 적용한다. 작업자 장애로 끝난 `EXPIRED`와 임대 시간이 지난 `ACTIVE`도 시도 횟수에 포함한다. 최신 수동 검토에 [명시적인 재개 기록](ai-reservation-recovery-review-resume.md)이 있으면 재개 시각부터 기존 간격을 적용하며 사용한 횟수는 초기화하지 않는다.
...
현재 간격과 최대 횟수는 운영값이 아니다. 공급자 응답 특성·요금·조회 제한을 확인한 뒤 값을 정해야 한다. 수동 검토 재개는 구현했지만 최대 횟수 도달을 해제하거나 횟수를 늘리는 명령은 없다.
...
전용 11건은 첫 시도, 활성 임대, 임대 만료 뒤 간격, 정확한 가능 시각 경계, 확인 실패·확인 완료의 재확인, 수동 검토와 명시적 재개, 완료·만료를 포함한 최대 횟수, 종료 예약, 일정·이력 입력 오류를 확인한다.

`test:ingestion` 70건과 내부 운영 조회·수동 검토 재개·작업 배정·배정 실행·제한 목록·작업 실행 기록·운영 중단·시도 연결을 포함한 전체 서버 341건이 실패·오류·건너뛰기 없이 통과했다. 전체 구성은 DB 없는 도메인 246건, 개발 API·계약 13건, PostgreSQL 예약·실행·복구·임대 갱신·heartbeat·연결·운영 조회·수동 검토 재개·작업 배정·제한 목록·작업 실행 기록/운영 중단/실행-시도 연결/기본 차단 82건이다. 화면·API 계약은 변경하지 않았고 Flyway V8까지 적용했다.

## 남은 작업

- 공급자 선정 뒤 실제 간격·최대 시도 횟수 확정
- 최대 횟수 도달을 별도로 해제해야 하는지 운영 정책 확정
