...
| Flyway V4 `ai_reservation_recovery_work_runs` | 제한 목록 실행 ID·작업자·조회 조건·상태·후보 결과 집계 저장 |
...
- 잘못된 임대 기간의 DB 제약
- 최신 수동 검토의 감사 재개와 동일 명령 재전달·ID 충돌
- 예약 스냅샷 변경과 최신 시도 펜싱
...
저장소 전용 16건, 복구 조정자 16건, 재확인 정책 11건, 내부 운영 조회·작업 배정·재개 연결 11건, 제한 목록·작업 실행 기록·운영 중단·시도 연결 18건과 전체 서버 341건이 실패·오류·건너뛰기 없이 통과했고 빌드도 성공했다. 전체 구성은 DB 없는 도메인 246건, 개발 API·계약 13건, PostgreSQL 예약·실행·복구·임대 갱신·heartbeat·연결·운영 조회·수동 검토 재개·작업 배정·제한 목록·작업 실행 기록/운영 중단/실행-시도 연결/기본 차단 82건이다.
