...
## 수집 조회 추가 검토 — 2026-09-06, 미적용

`007dc7e`에서 기존 13개 항목을 제외하고 수집·회원 호출 경로를 확인했다. 추가 후보는 다음 2개다. 새 라이브러리 없이 기존 JDBC 조회를 정리할 수 있다.

| 우선순위 | 현재 문제 | 개선 방향 |
|---|---|---|
| 중간 | [수집 페이지 조회](../../backend/src/main/java/kr/youthpolicymate/ingestion/OntongCollectionStore.java)의 `page`가 항상 원문을 포함한 전체 행을 반환한다. 정상 재처리에서 `OntongSweepStore.claim`, `OntongCollectionService.applyStored`, `OntongSweepRunner.tick`이 같은 원문을 각각 읽지만 실제 해석은 `applyStored`만 한다. | 상태·항목 수·원문 존재 여부만 읽는 조회를 분리한다. 실제 해석할 때만 원문을 가져오면 정상 재처리의 원문 읽기를 3회에서 1회로 줄일 수 있다. 저장 원본 재처리·부분 실패·현재 상태 확인은 유지한다. |
| 낮음 | 같은 저장소의 `startDispatch`가 제어 행을 잠그기 위해 ID를 조회한 뒤, 한도 설정 시 `reserved_run_id`를 다시 조회한다. 같은 트랜잭션에서 이미 잠근 행의 중복 읽기다. | 처음 `FOR UPDATE` 조회에서 예약 실행 ID도 반환한다. 한도 설정 경로의 해당 SELECT 2회를 1회로 줄인다. 예약 교체 거절·발송 시작 중복 차단·호출 간격 갱신은 유지한다. |

첫 항목은 원문을 가져오는 횟수의 개선이며 전체 SELECT 수를 3회에서 1회로 줄인다는 뜻은 아니다. HTTP 응답 상한은 1 MiB이고 실제 저장 원문의 크기에 따라 절감량이 달라진다. 두 항목 모두 코드 경로로 확인했으며 실행 시간·전송량을 측정하지 않았다.

구현 시 기존 `OntongCollectionTest`·`OntongSweepTest`의 재처리·동시 배정·예약 교체·발송 중복·호출 간격 검사를 사용한다. 고정 정규식의 상수화나 짧은 검증 함수 공통화는 별도 작업으로 잡을 만큼 이득이 크지 않아 제외했다. 이번 검토는 문서만 변경해 앱 테스트를 실행하지 않았다.
