...
2026-09-12 기준. `/conditions`에서 기본 조건을 확인한 뒤 정책명·내용으로 검색하고 연령 조건 충족 우선 또는 최근 수집순으로 정렬한다.
...
| 미래 청년 일자리 5월 모집 | 1986.1.1.~2007.12.31. 출생 | 상한 초과 시 군복무 연장 확인, 신청 당시 주소·근로·재학·사업 이력 등 |

정책별 추가 질문과 같은 연령 비교 함수를 사용한다. 기준의 출처는 [응시료](exam-fee-questions.md), [K-패스](kpass-questions.md), [청년정책네트워크](seoul-youth-network-questions.md), [이사비](moving-fee-questions.md), [청약통장](youth-housing-savings-questions.md), [저축계좌](youth-tomorrow-savings-questions.md), [햇살론유스](haetsalron-youth-questions.md), [청년 미래이음 대출](miso-youth-future-questions.md), [미래 청년 일자리](future-youth-jobs-questions.md)에 있다. 검토된 원문 해시가 일치하고 각 규칙의 2026년 적용 기간 안에 있을 때만 적용한다. 검토되지 않은 정책·바뀐 원문·다음 해에는 연령을 미확인으로 남긴다. 국가근로장학금·보증료 지원은 기본 조건만으로 비교하지 않는다.
...
- `PolicyRuleDefinition`에서 만든 `PolicyAgeComparison`을 SQL 정렬과 응답 표시에 함께 사용한다. 정책별 추가 조회 없이 규칙 조회 한 번과 건수·페이지 조회 두 번으로 처리한다. 현재 [공고별 조건 데이터](policy-rule-data.md)로 9개 정책의 연령을 비교하고 생년월일 답변을 재사용한다.
...
미래 청년 일자리 추가 시 출생일 경계·군복무 연장 미확인·공고일·연도 만료·원문 변경을 확인했다. PostgreSQL 정책 API·계약 일치·실제 기본 조건 검색도 확인했다. 최신 명령·로그는 [미래 청년 일자리 검증](future-youth-jobs-questions.md#연결과-검증)에 있다. 기존 정책의 코드가 같은 검증 결과는 재사용한다.
...
현재 실행 명령·JDK 경로·로그는 [미래 청년 일자리 검증](future-youth-jobs-questions.md#연결과-검증)에 있다. 정기 수집·알림·이메일 발송은 비활성화이며 다음 작업에서 프로세스 상태를 확인한다.
