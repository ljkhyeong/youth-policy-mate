...
개발 전용 화면에서 인공 마감 자료와 후보 날짜를 표시할 수 있다. 날짜형과 시각형 마감, 오늘 후보의 발송 시각 확인 필요, 후보 없음 사유를 구분한다. 화면에서 날짜 계산을 다시 구현하거나 실제 저장·예약이 완료된 것으로 표시하지 않는다. 실제 API·회원·발송 연결은 별도다.
