...
실제 저장·해제·수신 동의·예약·발송·Outbox·DB·API·화면 연결은 없다. 최신 자료 선택, 이전 예약 취소, 실제 발송 시각과 이미 지난 시점의 판정은 해당 기능 구현 때 처리한다. 후보 계산의 빈 목록만으로 기존 예약이 취소되었다고 간주하면 안 된다.
...
개발 전용 [마감·알림 후보 미리보기](deadline-reminder-preview.md)를 추가했다. 고정 인공 자료로 날짜형·시각형 마감, 오늘 후보의 발송 시각 확인 필요, 후보 없음 사유를 구분한다. 화면은 날짜를 계산하지 않으며 이 서버 모델과도 아직 연결하지 않았다. 개발 전용 조회 API의 계약·생성 타입을 정한 뒤 서버 결과를 연결한다. 실제 정책·회원·발송 연결은 별도다.
