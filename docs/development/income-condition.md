...
원문 해석, 소득 산정, 원천 DTO, API·저장·화면과의 판정 연결은 구현하지 않았다. 실제 정책이나 `/conditions`의 입력을 연결하지 않는다. 별도의 [개발 전용 소득 질문 미리보기](income-question-preview.md)도 서버를 호출하지 않는다. 아래 모델은 내부 호출 계약이며 외부 요청을 바로 바인딩할 DTO가 아니다.
...
- [개발 전용 소득 질문 미리보기](income-question-preview.md)에서 정의·기간·원화 단위, 구간·모름·별도 인공 상황의 추가 확인 안내를 점검할 수 있다. 서버 비교기 호출·실제 재질문 연결은 아직 없다.
