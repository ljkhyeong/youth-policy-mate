...
`test:ingestion`은 수집 진행·AI 후보·실행/복구 결과 연결·사전 판단·복구 재확인 정책을 실행한다. 페이지·항목별 시도, 명시적 종료·부분 실패, 중단·재개·늦은 결과, AI 버전·재사용·실행·복구 응답의 현재 개정 재검사, 사전 비용과 수동 검토 재개 후 간격·최대 횟수를 확인한다. 인증키·DB·Docker 없이 실행하며 예약·정산은 아래 DB 검사로 분리한다.
