...
`test:web`은 비회원 조건 입력 검사 5개·상태 화면 검사 3개·추가 질문 표시 검사 4개, 총 12개를 실행한다. 필수 항목·날짜·선택지·연령 제한 비적용·서울 자정 경계에 더해 상태 안내 역할, 오류 원문 비노출과 404 복귀 경로, 인공 질문의 미선택 상태·답변 의미·모름 처리를 확인한다. API 인증키·Docker·네트워크가 필요하지 않다.
