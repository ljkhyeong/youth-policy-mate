...
`FAILED`와 `UNKNOWN`을 자동 재발송하지 않는다. 확인 메일은 사용자가 새 코드를 요청할 수 있다. 정책 메일의 수동 재발송·공급자 접수 조회 화면은 아직 없다. `SENDING`에서 중단된 작업은 처리기 재시작 후 `UNKNOWN`으로 옮긴다. 같은 배정의 늦은 응답과 Resend 웹훅을 반영하며 새 발송을 만들지 않는다. 외부 이메일의 정확히 한 번 전달을 보장하지 않는다.
