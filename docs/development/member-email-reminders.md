...
2026-09-12 구현 기준이다. Resend API·웹훅과 운영 설정은 [외부 API 연결 안내](external-api-runtime.md)를 참고한다. 제품 동작은 [PRD](../PRD/0001_product-baseline/spec.md), 회원 로그인은 [회원 연결 안내](member-policy-flow.md)를 따른다.
...
- 정책 메일의 [본문 링크·표준 수신 해제 버튼](email-unsubscribe.md)에서도 로그인 없이 알림을 끌 수 있다. 주소 변경과 재동의 후에는 이전 링크가 현재 동의를 바꾸지 않는다.
- 서버의 외부 발송 배정 전에 취소가 완료되면 보내지 않는다. 이미 발송을 시작한 요청은 회수할 수 없다.
- 발송 설정이 없으면 준비 중이라고 표시한다. 기존 주소의 수신 해제·삭제는 발송 설정이 없어도 가능하다.
- 화면의 [설정 오류 복구](email-settings-recovery.md)는 조회 중 변경을 막고, 응답 유실 후 이전 설정을 숨겨 재조회한다. 재조회만으로 메일을 다시 보내거나 수신 동의를 바꾸지 않는다.
...
키는 재시작 후에도 같은 값을 유지해야 기존 주소를 읽을 수 있다. 키 교체·재암호화 도구는 아직 없다. 운영 보관 기간·계정 삭제 정책과 실제 공급자 수신 확인은 공개 전 남은 작업이다. 비밀 값·주소·확인 코드를 로그나 문서에 넣지 않는다.

기존 서버 실행 방법을 사용하며 Flyway V17이 이메일 설정과 발송 대기 테이블을 만들고 V29가 공급자 결과와 발송 중단 사유를 추가한다. 수집 CLI는 이메일 발송 설정을 명시적으로 끄고 실행한다. `preview`에서는 회원 이메일 저장·발송 구성 요소를 만들지 않는다.
...
| `PENDING` | 발송 전, 취소 가능 |
| `SENDING` | 외부 발송 배정 완료 |
| `SENT` | 공급자 발송 요청 접수. 수신함 도착 보장은 아님 |
...
`FAILED`와 `UNKNOWN`을 자동 재발송하지 않는다. 확인 메일은 사용자가 새 코드를 요청할 수 있다. 공급자 발송 ID가 있는 Resend 요청은 [관리자 상태 조회](email-provider-status.md)를 제공하며, 정책 메일의 수동 재발송은 제공하지 않는다. `SENDING`에서 중단된 작업은 처리기 재시작 후 `UNKNOWN`으로 옮긴다. 같은 배정의 늦은 응답과 Resend 웹훅을 반영하며 새 발송을 만들지 않는다. 외부 이메일의 정확히 한 번 전달을 보장하지 않는다.
