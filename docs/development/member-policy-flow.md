...
- 실제 원문의 기준일·필수 조건·예외를 판정 규칙으로 확정하지 않았으므로 결과는 모두 `NEEDS_REVIEW`다. 입력에 따른 신청 가능 판정이나 개인별 추천 순위는 아직 제공하지 않는다.
...
공개 `POST /api/v1/policies/checks`는 저장 없는 조건 확인이므로 CSRF 예외다. 나머지 회원 변경에는 `/api/v1/session`에서 받은 `csrfToken`을 `X-CSRF-TOKEN`에 넣는다. `/api/v1/me/**`는 회원 역할과 서버 세션의 소유자 식별자를 사용한다. 로그아웃은 `POST /api/v1/logout`이다. Next.js 중계는 허용한 경로만 고정된 서버로 보내고 세션 쿠키·CSRF 외의 인증 헤더를 전달하지 않는다.
...
2. 실제 정책의 원문 기준일·조건·예외를 판정 규칙으로 연결하고 필요한 추가 질문을 제공한다.
