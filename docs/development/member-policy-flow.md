...
현재 구현 범위다. 제품 동작은 [PRD](../PRD/0001_product-baseline/spec.md), 모듈 책임은 [ADR-0001](../ADR/0001_기술스택과_책임_분리.md)을 따른다.
...
- `/login`: 로그인 방법 조회 실패 시 다시 불러오기를 제공한다. 설정된 카카오·네이버만 표시하며 앱 설정이 없으면 공개 탐색을 안내한다. 로그인 후 현재 링크의 정책번호·관리자 경로·내 정책 화면으로 복귀하고, 이미 로그인한 경우에도 같은 이동 버튼을 제공한다. 일반 로그인은 이전 이동 기록을 지우며 제공자 로그인 실패 후 재시도는 직전 복귀 주소를 유지한다. 로그인만으로 정책을 저장하지 않는다.
...
- [내 정책 화면 이동](member-navigation.md): 탭·일정 필터·이메일 설정 펼침 상태를 주소로 복원한다. 수신 해제 안내에서 이메일 설정을 바로 열고 로그인 후에도 같은 화면으로 돌아간다.
- 관심 정책과 마감 일정은 [정책명 검색·변경 필터](#저장한-정책-검색)를 제공한다. 검색은 현재 저장 목록에 적용하며 추가 회원 조회나 변경 확인 처리를 하지 않는다.
...
2026-09-12, 코드 `bae7b16`.

- 정책명 부분 일치로 검색하며 영문 대소문자는 구분하지 않는다. 검색어는 앞뒤 공백을 제거하고 최대 80자로 받는다. 설명·신청기간은 검색 대상에 포함하지 않는다.
- ‘변경된 정책만 보기’는 저장 당시 개정과 현재 개정이 다른 정책을 표시한다. 검색어·변경 필터·일정의 접수 상태를 함께 적용하고 서버의 정렬 순서를 유지한다.
- 검색 버튼이나 Enter로 검색어를 적용한다. 변경 필터를 누르면 입력 중인 검색어도 함께 적용한다. 일정의 접수 상태 선택은 기존처럼 즉시 적용한다.
- 검색 결과 수와 저장한 전체 건수를 표시한다. 검색 결과 없음과 저장한 정책 없음을 구분하고, 초기화 후 검색 입력으로 초점을 옮긴다.
- `q`와 `changed=1`을 주소·로그인 복귀에 포함한다. 검색 초기화는 검색어·변경 필터와 현재 일정의 접수 상태를 지우며 알림 페이지·필터는 유지한다. 관심 정책 탭에서는 숨겨진 일정 필터도 유지한다.
- 저장 해제 후 재조회·목록 오류 복구에서도 적용한 검색 조건을 유지한다. 검색이나 필터 변경으로 API를 추가 호출하거나 저장 상태를 바꾸지 않는다.

| 검증 | 결과 |
|---|---|
| `npm run verify -- test:web -- src/features/member/member-location.test.ts src/features/member/member-policy-list.test.tsx src/features/member/login-destination.test.ts` | 검색·필터 조합·서버 순서·빈 결과 구분·주소·로그인 복귀 통과. `.local/verification/1789217899628-411b43c8.log` |
| `npm run verify -- check:web` | 린트·타입 통과. `.local/verification/1789217977064-6d2efdd2.log` |
| `npm run verify -- build:web` | 버튼 정렬 보완을 포함한 최종 웹 빌드 통과. `.local/verification/1789218540026-4c38876f.log` |
| 전용 헤드리스 브라우저 | 검색·변경/일정 필터 조합·초기화·이전/다음·새로고침·새 탭·저장 해제 응답 유실·조회 실패·로그인 복귀·키보드·빈 저장 목록 통과. `/tmp/youth-saved-policy-search/search-final.log` |
| 화면 정렬 | 390px·1280px에서 버튼 높이·정렬과 가로 넘침을 확인했다. `/tmp/youth-saved-policy-search/visual.log` |

기존 3000 포트 서버와 모의 회원·OAuth 응답을 사용했다. 검색 중 추가 목록 조회가 없었고 저장 해제 1건은 모의 처리했다. 실제 회원 변경·외부 공급자 호출은 없었다. 최초 브라우저 검증은 주소 반영 전 체크 상태를 검사해 중단됐고, 이동 완료를 기다리도록 수정한 뒤 통과했다. 이후 검색 버튼의 공통 위쪽 여백을 보완해 화면 정렬과 웹 빌드만 다시 확인했다. 스크립트·캡처는 `/tmp/youth-saved-policy-search/`에 있으며 전용 브라우저를 종료했다. 서버·DB·API 계약·운영 설정과 이미지 빌드는 변경하거나 실행하지 않았다.
