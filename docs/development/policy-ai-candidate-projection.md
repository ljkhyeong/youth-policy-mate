...
현재 `test:ingestion` 70건과 전체 서버 334건이 실패·오류·건너뛰기 없이 통과했다. 전체 구성은 DB 없는 도메인 246건, 개발 API·계약 13건, PostgreSQL 예약·실행·복구·연결·운영 조회·수동 검토 재개·작업 실행/기본 차단 75건이다. 화면·API 계약은 변경하지 않았고 Flyway V7까지 적용했다.
