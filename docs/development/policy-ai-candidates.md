...
`test:ai-candidates`는 AI 후보를, `test:ai-candidate-projection`은 결과 연결을 검사한다. `test:ingestion`은 여기에 [사전 판단](ai-request-admission.md)과 [복구 재확인 정책](ai-reservation-recovery-retry-policy.md)을 포함한다. 이 단위 검사에는 인증키·DB·Docker가 필요하지 않다. 실제 수집은 `test:policy-collection`, 예약·정산은 `test:ai-reservations`로 검사하며 Docker가 필요하다.
