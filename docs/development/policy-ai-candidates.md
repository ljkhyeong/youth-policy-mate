...
`test:ai-candidates`는 전용 12건, `test:ingestion`은 기존 수집 진행 12건과 AI 후보 12건, 총 24건을 선택한다. 두 단위 검사에는 인증키·DB·Docker가 필요하지 않다. 전체 빌드는 실제 PostgreSQL 통합 테스트 때문에 Docker가 필요하다.
...
- `BUDGET_LIMIT`은 호출 측이 전달한 사유다. 비용 계산·상한 검사·외부 청구 차단을 구현한 것은 아니다. 후보 재사용 불가가 유료 호출 허용을 뜻하지 않는다.
