...
`test:ai-candidates`는 전용 12건, `test:ingestion`은 수집 진행 12·AI 후보 12·후속 [사전 판단](ai-request-admission.md) 14건, 총 38건을 선택한다. 두 단위 검사에는 인증키·DB·Docker가 필요하지 않다. 전체 빌드는 실제 PostgreSQL 통합 테스트 때문에 Docker가 필요하다.
...
- `BUDGET_LIMIT`은 호출 측이 전달한 사유다. 후속 [사전 판단 모델](ai-request-admission.md)에서 주어진 한도·사용액·예약액·최대 비용을 비교하지만, 이 사유 기록과 직접 연결하지 않았다. 실제 가격 계산·예약·외부 청구 차단은 없으며 후보 재사용 불가가 유료 호출 허용을 뜻하지 않는다.
