...
`preview`에서 마감 예시·자격 예시·명세의 GET 세 경로만 추가 허용한다. 쓰기 요청·다른 API·관리 정보 경로는 차단하고 기본 모드의 OpenAPI는 끈다. 개발 프로필을 운영에서 켜서는 안 되며 실제 회원 인증을 대신하지 않는다.
...
같은 생성 계약과 개발 전용 경계로 [자격 판정 결과·항목별 근거](eligibility-preview-api.md)도 서버 계산에 연결했다. 실제 원천 수집·사용자 조건 저장·회원 인증은 별도다. 성공 응답 확보 전에는 인공 자료 연동을 실제 정책 연동 완료로 취급하지 않는다.
