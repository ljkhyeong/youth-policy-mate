...
`238a28a` 기준. `/my`의 관심 정책·마감 일정에서 저장 이후 개정된 정책의 ‘변경 내용 보기’를 열면 저장 당시와 현재 내용을 비교한다.
...
`SavedPolicyChanges`는 비교를 펼칠 때만 조회한다. 접기·탭 전환·목록 갱신 시 이전 조회를 취소한다. 로딩·오류·변경 없음을 구분하고 재시도를 제공한다. 관리자 화면의 항목 비교를 `PolicyContentComparison`으로 분리해 재사용하며 기존 관리자 표시 항목은 유지한다. 데스크톱은 두 열, 모바일은 저장 당시·현재 순으로 쌓는다.
...
최종 코드 검증 후 변경은 문서뿐이다. 로컬 상태 조회 200·비회원 비교 API 401·내 정책 화면 200을 확인했다. 실제 회원 데이터는 변경하지 않았으며 실제 소셜 제공자 로그인·이메일 전달·AI 생성은 이번 검증에 포함하지 않았다.
