...
export function LoginPanel() {
  ...
  useEffect(() => {
    ...
    memberApi<MemberSession>("session", { signal: controller.signal }).then(result => {
      ...
      setSession(result);
      if (new URLSearchParams(window.location.search).has("error")) setError("로그인을 완료하지 못했어요. 다시 시도하거나 공개 정책을 둘러보세요.");
    }).catch(() => {
  ...
  return <section className="member-panel">
    ...
    {session?.authenticated ? <Link className="button-primary" href="/my">내 정책으로 이동</Link> : <>
      ...
      {session?.providers.length === 0 && <div className="availability-note"><div><strong>로그인 연결을 준비하고 있어요</strong><p>지금은 로그인 없이 정책 검색과 조건 확인을 이용할 수 있어요.</p></div></div>}
