...
export function LoginPanel({ admin = false, policy, loginFailed = false }: { admin?: boolean; policy?: string; loginFailed?: boolean }) {
  ...
  const resumeLogin = loginFailed && !admin && !policy;
  const destination = resumeLogin ? previousDestination : getLoginDestination(admin, policy);
  useEffect(() => {
  ...
  return <section ref={panel} className="member-panel">
    ...
    {session?.authenticated ? <a className="button-primary" href={destination}>{destination === "/admin/collection-exceptions" ? "수집 오류·보정 관리" : destination === "/my" ? "내 정책으로 이동" : "정책으로 돌아가기"}</a> : <>
      {session?.providers.map(provider => <a key={provider.id} className="button-primary button-block" href={provider.url}
        onClick={() => { if (!resumeLogin) rememberLoginDestination(admin, policy); }}>{provider.name}로 로그인</a>)}
