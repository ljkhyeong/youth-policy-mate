...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ next?: string | string[]; policy?: string | string[]; error?: string | string[] }> }) {
  ...
  return <SiteShell active="my"><main id="main-content" className="member-main"><header>
    ...
    <h1>{admin ? "관리자 계정으로 로그인하세요" : "로그인하고 정책을 저장하세요"}</h1>
    <p>{admin ? "로그인 후 관리자 권한을 확인합니다." : "로그인하면 내 조건과 관심 정책을 저장하고, 마감 일정과 알림을 확인할 수 있어요."}</p>
  </header><LoginPanel admin={admin} policy={policy} loginFailed={params.error !== undefined} /></main></SiteShell>;
}
