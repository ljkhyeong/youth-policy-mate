...
export default async function PoliciesPage({ searchParams }: { searchParams: Promise<{ q?: string; page?: string; questionsOnly?: string }> }) {
  ...
  return <SiteShell active="policies"><main id="main-content" className="policies-main">
    ...
    <p className="field-help">질문에 답하면 일부 신청 조건을 확인할 수 있어요. 최종 자격과 접수 기간은 별도로 확인해주세요.</p>
    <p className="policy-coverage">일부 정책만 제공해요. 찾는 정책이 없으면 <a href="https://www.youthcenter.go.kr/" target="_blank" rel="noopener noreferrer">온통청년<span className="sr-only"> (새 창)</span></a>에서 확인해주세요.</p>
    {result.status === "available" ? <>
      <div className="policy-results-heading"><p role="status">{query ? `‘${query}’ 검색 결과` : questionsOnly ? "조건 확인 질문이 있는 정책" : "전체 정책"} <strong>{result.data.total}건</strong></p>{(query || questionsOnly) && <Link href="/policies" className="text-link">검색·필터 초기화</Link>}</div>
      {result.data.items.length > 0 ? <div className="policy-list">{result.data.items.map(policy => <PolicyCard key={policy.policyNumber} policy={policy} />)}</div>
        ...
        : <PageState kind="empty" title="표시할 정책이 없어요" description={questionsOnly ? "검색 조건에 맞는 질문 제공 정책이 없어요. 검색어를 바꾸거나 필터를 해제해주세요." : "검색어를 바꾸거나 전체 목록을 확인해주세요. 아직 모든 정책을 제공하고 있지는 않아요."} actions={<Link href="/policies" className="button-secondary">전체 정책 보기</Link>} />}
