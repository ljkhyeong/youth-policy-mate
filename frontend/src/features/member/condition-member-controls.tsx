...
export function ConditionMemberControls({ input, onLoad, onSuggestBirthDate }: { input?: BasicConditions; onLoad: (value: ConditionDraft) => void; onSuggestBirthDate?: (value: string) => void }) {
  ...
  if (!session?.authenticated) return <p className="data-retention-note"><Link href="/login">로그인</Link>하면 입력한 조건을 저장하고 다시 불러올 수 있어요.</p>;
  return <div className="condition-member-controls">
    <p><strong>{session.displayName}님의 조건</strong> · 저장은 아래 버튼을 눌렀을 때만 해요.</p>
    <div className="form-actions">
      {input && <button type="button" className="button-primary" disabled={busy} onClick={() => act("save")}>내 조건 저장</button>}
      {!input && <button type="button" className="button-secondary" disabled={busy} onClick={() => act("load")}>저장한 조건 불러오기</button>}
      {!input && session.suggestedBirthDate && <p className="field-help">카카오 생년월일({session.suggestedBirthDate} · 양력)을 입력했어요. 본인 정보가 맞는지 확인해주세요.</p>}
      <button type="button" className="text-button" disabled={busy} onClick={() => act("clear")}>저장한 조건 삭제</button>
